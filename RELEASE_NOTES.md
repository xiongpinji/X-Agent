...
- Hardened final gate/evidence pack ordering: `--allow-missing-evidence-pack`
  ...
  path privacy checks. The gate still reports `ready_with_owner_gates` when
  owner-controlled external evidence is pending.
...
- Current fixed-point final gate status is `ready_with_owner_gates`; local
  gates are green, but owner-controlled provider, Feishu, GitHub test-resource,
  and hosted GitHub Actions evidence for the current commit SHA must be rerun
  with the owner-facing finalization command before RC tagging. Intermediate
  refreshes can temporarily report `ready_with_receipt_refresh_required` until
  receipt and evidence-pack reports are regenerated in order.
...
- GitHub Actions commercial RC workflow is defined locally but must still run
  successfully in GitHub before final RC tagging.
- Real OpenAI/DeepSeek/other provider smoke requires deployment-owner tokens;
  the current local Ollama provider attempt still requires model-storage repair
  or a replacement provider before it can clear the provider owner gate.
- Feishu and GitHub execute-mode acceptance requires test bot/test repository
  credentials and must be recorded before customer enablement.
