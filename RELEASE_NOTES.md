...
- RC aggregate test group passed with 299 tests.
...
- Deployment docs gate passed locally and preserves `ready_for_rc_tag`
  plus `full_parity_claimed=false` in owner-facing handoff docs.
...
- Final RC gate reports `ready_for_rc_tag` after owner-controlled provider,
  Feishu, GitHub test-resource, and hosted GitHub Actions gates are verified
