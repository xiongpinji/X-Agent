...
- Hardened final gate/evidence pack ordering: `--allow-missing-evidence-pack`
  ...
  path privacy checks. The fixed-point gate now reports `ready_for_rc_tag` after
  owner-controlled external evidence is verified.
...
- Current fixed-point final gate status is `ready_for_rc_tag`; local gates and
  owner-controlled provider, Feishu, GitHub test-resource, and hosted GitHub
  Actions evidence for commit `643a017b3a2ae00be212d186e2681a147b46bf6b` passed
  through the owner-facing finalization command. Intermediate refreshes can
  temporarily report `ready_with_receipt_refresh_required` until receipt and
  evidence-pack reports are regenerated in order.
- The existing pushed tag `x-agent-commercial-rc-20260608` resolves to
  `08cd6d114e0c0cb357ccea3e529aed7b2aea1045`, not the verified commit
  `643a017b3a2ae00be212d186e2681a147b46bf6b`. Treat this as a release
  consistency blocker until the owner creates a new tag at the verified commit
  or explicitly approves correcting the pushed tag.
