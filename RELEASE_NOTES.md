...
- Deployment docs gate passed locally and preserves `ready_with_owner_gates`
...
- Final RC gate reports `ready_with_owner_gates`, so candidate files can be
  staged after review but final RC tagging still needs owner-controlled
  provider, Feishu, and GitHub test-resource gates.
