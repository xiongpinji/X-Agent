...
FORBIDDEN_CONTAINS: tuple[ForbiddenPattern, ...] = (
    ...
    ForbiddenPattern(
        id="no_route_auth_audit_fail_open",
        description="Route auth audit must not be allowed to fail open in commercial RC CI.",
        token="python scripts/route_auth_audit.py --json || true",
    ),
    ForbiddenPattern(
    ...
    ForbiddenPattern(
        ...
        description="Production hardening gate must block commercial RC CI when production config is not hardened.",
        token="python scripts/production_hardening_gate.py --allow-blocked",
    ),
    ForbiddenPattern(
        id="no_production_hardening_gate_fail_open",
        description="Production hardening gate must not be allowed to fail open in commercial RC CI.",
        token="python scripts/production_hardening_gate.py || true",
    ),
)
