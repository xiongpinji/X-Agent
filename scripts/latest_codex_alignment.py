...
def _capabilities() -> list[CodexAlignmentCapability]:
    return [
        ...
        CodexAlignmentCapability(
            ...
            priority="P1",
            xagent_status="sdk_write_runner_safety_contract_ready",
            evidence=[
                "sdk_noninteractive_report",
                "sdk_contract_module",
                "sdk_cli_command",
            ...
            next_task="Implement the concrete owner-approved write SDK runner behind this safety plan after runtime safety review.",
            ...
            rationale="X-Agent now exposes SDK-style thread start/resume/run/read/evidence-read envelopes, non-interactive CLI JSON output, an owner-gated backend SDK invoke stub, a CLI HTTP adapter for read-only --execute, pending owner approval intent creation for write methods, approval handoff/readback commands, owner-approved write preflight, and a write runner safety plan/receipt contract; real write agent execution remains disabled.",
        ),
        CodexAlignmentCapability(
