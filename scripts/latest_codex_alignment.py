...
READY_XAGENT_STATUSES = frozenset(
    {
        ...
        "partial",
        "sdk_owner_acceptance_record_workflow_ready",
...
def build_evidence_specs(root: Path = ROOT, report_dir: Path = REPORT_DIR) -> tuple[AlignmentEvidenceSpec, ...]:
    return (
        ...
        AlignmentEvidenceSpec(
            ...
            "runtime_report",
            expected_statuses=frozenset({"sdk_owner_acceptance_record_workflow_ready"}),
...
def _capabilities() -> list[CodexAlignmentCapability]:
    return [
        ...
        CodexAlignmentCapability(
            ...
            priority="P1",
            xagent_status="sdk_owner_acceptance_record_workflow_ready",
            evidence=[
                "sdk_noninteractive_report",
                "sdk_contract_module",
                "sdk_cli_command",
            ...
            next_task="Add runtime enablement review contract that requires audit-backed owner acceptance evidence plus explicit disabled feature-flag review before considering the concrete write runner.",
