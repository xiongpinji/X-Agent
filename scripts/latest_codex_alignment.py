...
def _capabilities() -> list[CodexAlignmentCapability]:
    return [
        ...
        CodexAlignmentCapability(
            ...
            priority="P1",
            xagent_status="sdk_dry_run_executor_stub_ready",
            evidence=[
                "sdk_noninteractive_report",
                "sdk_contract_module",
                "sdk_cli_command",
            ...
            next_task="Implement the concrete owner-approved write SDK runner behind the audited dry-run stub after runtime safety review.",
            ...
            rationale="X-Agent now exposes SDK-style thread start/resume/run/read/evidence-read envelopes, non-interactive CLI JSON output, an owner-gated backend SDK invoke stub, a CLI HTTP adapter for read-only --execute, pending owner approval intent creation for write methods, approval handoff/readback commands, owner-approved write preflight, a write runner safety plan/receipt contract, and an audited dry-run executor stub; real write agent execution remains disabled.",
        ),
        CodexAlignmentCapability(
