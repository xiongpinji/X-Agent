...
class PWAManager {
  ...
  async requestInstall(): Promise<boolean> {
    // BeforeInstallPromptEvent is not in the TS DOM lib version we target.
    const event = (window as Window & { deferredPrompt?: { prompt: () => void; userChoice: Promise<{ outcome: string }> } }).deferredPrompt;
  ...
  async requestBackgroundSync(tag: string): Promise<void> {
    if ('serviceWorker' in navigator && 'SyncManager' in window) {
      try {
        ...
        // Background Sync manager is not in the TS DOM lib version we target.
  ...
  async requestPeriodicSync(tag: string, minInterval: number): Promise<void> {
    if ('serviceWorker' in navigator && 'PeriodicSyncManager' in window) {
      try {
        const registration = await navigator.serviceWorker.ready;
        // PeriodicSyncManager is not in the TS DOM lib version we target.
        const periodicRegistration = registration as ServiceWorkerRegistration & {
          periodicSync: { register: (tag: string, options: { minInterval: number }) => Promise<void> };
        };
        await periodicRegistration.periodicSync.register(tag, { minInterval });
      } catch (error) {
