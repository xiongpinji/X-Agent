...
export function MemoryDetailPage(props: MemoryDetailPageProps) {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        const response = await fetch(`/api/v1/memory-control/detail/${encodeURIComponent(memoryId)}`, { method: "GET", headers: { "Content-Type": "application/json" } });
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <Panel title="快捷操作">
          <div className="grid gap-2">
            <button className="border-b px-3 py-2 text-left hover:bg-gray-50">查看引用关系</button>
            <button className="border-b px-3 py-2 text-left hover:bg-gray-50">查看经验关联</button>
            <button className="border-b px-3 py-2 text-left hover:bg-gray-50">查看历史记录</button>
          </div>
        </Panel>
      </section>
    </div>
