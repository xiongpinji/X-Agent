...
export function MemoryHistoryPage(props: MemoryHistoryPageProps) {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        const response = await fetch("/api/v1/memory-control/history", { method: "GET", headers: { "Content-Type": "application/json" } });
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <Panel title="操作区">
          <div className="grid gap-2">
            <button className="border-b px-3 py-2 text-left hover:bg-gray-50">筛选成功事件</button>
            <button className="border-b px-3 py-2 text-left hover:bg-gray-50">筛选失败事件</button>
            <button className="border-b px-3 py-2 text-left hover:bg-gray-50">导出历史记录</button>
          </div>
        </Panel>
      </section>
    </div>
