    ...
    <div className="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)_300px]">
      <aside className="console-section">
        <header className="flex items-start justify-between gap-3">
          ...
          <button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onSelectConversation(activeConversation?.conversation_id ?? props.conversations[0]?.conversation_id ?? "")}>返回当前</button>
        </header>
          ...
          <>
            <header className="border-b pb-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onOpenRoom?.(activeConversation.room_id ?? activeConversation.conversation_id)}>
                  打开关联房间
                </button>
              </div>
            </header>
  ...
  return (
    <div className="space-y-3">
      {messages.length ? messages.map((message) => {
        ...
        return (
          <div key={message.message_id} className={`border-b py-2${isMine ? " bg-blue-50" : ""}`}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button className="border px-2 py-1 text-xs hover:bg-gray-50" onClick={() => onOpenAudit?.(message.message_id)}>查看审计</button>
              <button className="border px-2 py-1 text-xs hover:bg-gray-50" onClick={() => onOpenMessageThread?.(message.message_id)}>查看线程</button>
              {message.room_id ? <button className="border px-2 py-1 text-xs hover:bg-gray-50" onClick={() => onOpenRoom?.(message.room_id ?? "")}>打开房间</button> : null}
            </div>
          </div>
        );
      }) : <div className="empty-state text-center">这个对话还没有消息</div>}
    </div>
  );
}
...
function ChatPresencePanel({ presence }: { presence: PresenceMap }) {
  return <section><h3 className="font-semibold">在线状态</h3><div className="mt-3 space-y-2">{Object.entries(presence).length ? Object.entries(presence).map(([agentId, info]) => <div key={agentId} className="border-b px-3 py-2 text-sm"><div className="font-medium">{agentId}</div><div className="text-xs text-gray-500">{info.status ?? "unknown"}</div></div>) : <div className="empty-state">暂无在线数据</div>}</div></section>;
}

function ChatMemberPanel({ conversation, avatars }: { conversation: ConversationSummary | null; avatars: RoleAvatar[]; }) {
  return <section><h3 className="font-semibold">成员</h3><div className="mt-3 space-y-2">{conversation?.participant_ids?.length ? conversation.participant_ids.map((participantId, index) => <div key={participantId} className="flex items-center gap-3 border-b px-3 py-2"><div className="h-10 w-10 rounded-full bg-gray-200" /><div><div className="text-sm font-medium">{participantId}</div><div className="text-xs text-gray-500">{avatars[index]?.display_name ?? "角色成员"}</div></div></div>) : <div className="empty-state">暂无成员</div>}</div></section>;
}

function ChatReferencePanel() {
  return <section><h3 className="font-semibold">引用</h3><div className="mt-3 border p-3 text-sm text-gray-500">暂未接入引用面板</div></section>;
}
