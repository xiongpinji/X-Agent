    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="console-section">
        ...
        <div>
          {props.roleCatalog.templates.map((template) => (
            <button key={template.role_id} className={`w-full border-b px-3 py-3 text-left transition hover:bg-gray-50 ${props.selectedRoleTemplateId === template.role_id ? "border-blue-500 bg-blue-50" : ""}`} onClick={() => props.onSelectRoleTemplate(template.role_id)}>
      ...
      <aside className="console-section space-y-4">
        {selectedTemplate ? <>
          ...
          <section><h4 className="font-medium">工作流预览</h4><ul className="mt-3 space-y-2">{props.roleCatalog.workflows.filter((workflow) => workflow.role_template_id === selectedTemplate.role_id).flatMap((workflow) => workflow.steps).map((step) => <li key={step} className="border-b px-3 py-2 text-sm">{step}</li>)}</ul><button className="mt-3 border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onOpenWorkflow?.(selectedTemplate.role_id)}>打开工作流</button></section>
          <section><h4 className="font-medium">能力与工具</h4><div className="mt-3 flex flex-wrap gap-2">{selectedTemplate.core_skills.map((skill) => <span key={skill} className="rounded-full bg-gray-100 px-3 py-1 text-xs">{skill}</span>)}</div><div className="mt-3 flex flex-wrap gap-2">{selectedTemplate.tools.map((tool) => <span key={tool} className="rounded-full bg-gray-100 px-3 py-1 text-xs">{tool}</span>)}</div><button className="mt-3 border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onOpenTools?.(selectedTemplate.role_id)}>查看工具映射</button></section>
