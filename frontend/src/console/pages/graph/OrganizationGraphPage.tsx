      ...
      <section className="console-section">
        ...
        <div className="space-y-4">
          {props.graph.departments.map((department) => {
            ...
            return (
              <div key={department.department_id} className="console-section">
                ...
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {departmentAgents.map((agent) => {
                    ...
                    return (
                      <button key={agent.agent_id} className={`border p-3 text-left hover:bg-gray-50 ${props.selectedNodeId === agent.agent_id ? "border-blue-500 bg-blue-50" : ""}`} onClick={() => props.onSelectNode(agent.agent_id)}>
                  ...
                  {departmentRooms.map((room) => (
                    <button key={room.room_id} className={`border p-3 text-left hover:bg-gray-50 ${props.selectedNodeId === room.room_id ? "border-blue-500 bg-blue-50" : ""}`} onClick={() => props.onSelectNode(room.room_id)}>
                      ...
                      <div className="mt-3 flex gap-2">
                        <button className="border px-2 py-1 text-xs hover:bg-gray-50" onClick={(e) => { e.stopPropagation(); props.onCreateRoomFromNode?.(room.room_id); }}>打开房间</button>
                      </div>
                    </button>
                  ))}
                </div>
      ...
      <aside className="console-section">
        ...
        {selectedNode ? (
          <div className="mt-4 space-y-4">
            ...
            {selectedAgent ? <section className="border p-3"><div className="text-sm font-medium">智能体信息</div><div className="mt-2 text-sm text-gray-600">岗位：{selectedAgent.title}</div><div className="text-sm text-gray-600">在线：{selectedAgent.online_status}</div><div className="mt-3 flex flex-wrap gap-2">{selectedAgent.capabilities.map((cap) => <span key={cap} className="rounded-full bg-blue-50 px-2 py-1 text-xs text-blue-700">{cap}</span>)}</div><div className="mt-3 flex gap-2"><button className="bg-blue-600 px-3 py-2 text-sm text-white" onClick={() => props.onCreateAgentFromNode?.(selectedAgent.agent_id)}>从节点创建智能体</button></div></section> : null}
            {selectedRoleTemplate ? <section className="border p-3"><div className="text-sm font-medium">岗位模板</div><div className="mt-1 text-sm text-gray-600">{selectedRoleTemplate.description}</div><div className="mt-3 flex flex-wrap gap-2">{selectedRoleTemplate.core_skills.map((skill) => <span key={skill} className="rounded-full bg-gray-100 px-2 py-1 text-xs text-gray-700">{skill}</span>)}</div></section> : null}
            {selectedAvatar ? <section className="border p-3"><div className="text-sm font-medium">角色形象</div><div className="mt-2 flex items-center gap-3"><div className="h-12 w-12 rounded-full bg-gray-200" /><div><div className="text-sm font-medium">{selectedAvatar.display_name}</div><div className="text-xs text-gray-500">{selectedAvatar.style}</div></div></div></section> : null}
            <div className="flex gap-2"><button className="bg-blue-600 px-3 py-2 text-sm text-white" onClick={() => props.onCreateAgentFromNode?.(selectedNode.node_id)}>从节点创建智能体</button><button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onCreateRoomFromNode?.(selectedNode.node_id)}>打开关联房间</button></div>
          </div>
