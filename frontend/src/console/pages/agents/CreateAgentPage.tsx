    ...
    <div className="grid gap-4 lg:grid-cols-[360px_minmax(0,1fr)_320px]">
      ...
      <section className="console-section">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="会议室"><select className="w-full border px-3 py-2" value={form.roomId} onChange={(e) => update("roomId", e.target.value)}><option value="">不绑定</option>{props.organizationGraph.meeting_rooms.map((room) => <option key={room.room_id} value={room.room_id}>{room.name}</option>)}</select></Field>
          <Field label="人格风格"><input className="w-full border px-3 py-2" value={form.persona} onChange={(e) => update("persona", e.target.value)} /></Field>
          <Field label="语气"><input className="w-full border px-3 py-2" value={form.tone} onChange={(e) => update("tone", e.target.value)} /></Field>
