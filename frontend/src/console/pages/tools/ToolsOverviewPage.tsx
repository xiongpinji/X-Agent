...
export function ToolsOverviewPage(props: ToolsOverviewPageProps) {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        const response = await fetch("/api/v1/tools-control/overview", { method: "GET", headers: { "Content-Type": "application/json" } });
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <Panel title="快捷入口">
          <div className="grid gap-2">
            <button className="border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => props.onOpenDetail?.()}>查看工具详情</button>
            <button className="border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => props.onOpenPlugins?.()}>进入插件管理</button>
