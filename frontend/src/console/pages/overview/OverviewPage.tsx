...
import type { RealtimeSnapshot } from "../../state/consoleReducer";
      ...
      <header className="console-page-header">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onOpenAction?.("open_search")}>全局搜索</button>
            <button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onOpenAction?.("open_audit")}>统一审计</button>
            <button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onOpenAction?.("open_execution")}>运行控制</button>
            <button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onOpenAction?.("open_tools")}>工具中心</button>
            <button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onOpenAction?.("open_memory")}>记忆中心</button>
            <button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onOpenAction?.("open_org")}>组织权限</button>
            <button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => props.onOpenAction?.("open_market")}>能力市场</button>
          </div>
        </div>
      </header>
          ...
          <Card title="待处理事项">
            <div className="console-kpi-row">
              {pendingItems.map((item) => (
                <button key={item.key} className="console-kpi text-left" onClick={() => props.onOpenPendingItem?.(item.key) ?? props.onOpenAction?.(item.action)}>
                  <span className="kpi-label">{item.label}</span>
                  <span className="kpi-value">{item.value}</span>
                  <span className="text-xs opacity-50">对应告警：{item.alertKey}</span>
                </button>
              ))}
            </div>
          </Card>
          ...
          <Card title="平台入口">
            <div className="divide-y">
              {platformEntries.map((entry) => (
                <button key={entry.label} className="w-full px-3 py-2 text-left hover:bg-gray-50" onClick={() => props.onOpenAction?.(entry.action)}>
            ...
            <Card title="最近访问">
              <div className="space-y-2">
                {recentEntries.map((entry) => (
                  <button key={entry.label} className="w-full border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => props.onOpenAction?.(entry.action)}>
                    <div className="font-medium">{entry.label}</div>
...
function MetricCard({ label, value, description: _description, onClick }: { label: string; value: string; description?: string; onClick?: () => void }) {
  return (
    <button className="console-kpi text-left" onClick={onClick}><span className="kpi-label">{label}</span><span className="kpi-value">{value}</span></button>
  );
}
  ...
  return (
    <Card title="调度建议">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => onExecuteAction?.("open_workflow")}>查看工作流</button>
        <button className="border px-3 py-2 text-sm hover:bg-gray-50" onClick={() => onExecuteAction?.("open_audit")}>查看审计</button>
      </div>
    </Card>
  );
}
    ...
    <Card title="推荐动作">
      <div className="space-y-2">
        {actions.length ? actions.map((action) => (
          <button key={action.action} className="w-full border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => onExecuteAction?.(action.action)}>
    ...
    <Card title="在线智能体">
      <div className="space-y-2">
        {agents.length ? agents.map((agent) => {
          ...
          return (
            <button key={agent.agent_id} className="flex w-full items-center gap-3 border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => onOpenAgent?.(agent.agent_id)}>
    ...
    <Card title="会议室">
      <div className="space-y-2">
        {rooms.length ? rooms.map((room) => (
          <button key={room.room_id} className="w-full border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => onOpenRoom?.(room.room_id)}>
    ...
    <Card title="最近消息">
      <div className="space-y-3">
        {messages.length ? messages.map((msg) => {
          ...
          return (
            <button key={msg.message_id} className="w-full border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => msg.conversation_id && onOpenConversation?.(msg.conversation_id)}>
    ...
    <Card title="最近对话">
      <div className="space-y-2">
        {conversations.length ? conversations.map((conversation) => (
          <button key={conversation.conversation_id} className="w-full border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => onOpenConversation?.(conversation.conversation_id)}>
  ...
  return (
    <Card title="风险提示">
      <div className="space-y-2 text-sm text-gray-600">
        {alerts.map((alert) => (
          <button key={alert.label} className="w-full border-b px-3 py-2 text-left hover:bg-gray-50" onClick={onOpenAudit}>
            <div className="font-medium text-gray-900">{alert.label}</div>
            <div className="text-xs text-gray-500">{alert.detail}</div>
          </button>
        ))}
      </div>
      <button className="mt-3 border px-3 py-2 text-sm hover:bg-gray-50" onClick={onOpenAudit}>查看审计</button>
    </Card>
  );
}
...
function SystemQuickActions({ onOpenAction }: { onOpenAction?: (actionKey: string) => void; }) {
  return <Card title="快捷入口"><div className="grid gap-2"><button className="border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => onOpenAction?.("open_workflow")}>打开工作流</button><button className="border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => onOpenAction?.("open_audit")}>打开审计</button><button className="border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => onOpenAction?.("open_rooms")}>打开会议室</button><button className="border-b px-3 py-2 text-left hover:bg-gray-50" onClick={() => onOpenAction?.("open_chat")}>打开实时通讯</button></div></Card>;
}
