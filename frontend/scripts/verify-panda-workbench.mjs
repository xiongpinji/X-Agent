...
assert(manifest.bff?.resourcesFlagDefault === 'false', 'Panda manifest must keep resources BFF disabled by default')
...
assert(manifest.frontendCloseout?.currentPhase === 'non-blocking-frontend-closeout', 'Panda manifest must expose the frontend closeout phase')
assert(manifest.frontendCloseout?.backendDependency === 'resources-bff-and-page-specific-data', 'Panda manifest must record the backend dependency for frontend closeout')
...
assert(manifest.frontendHandoffGates?.strictBackendGate === 'pending-backend', 'Panda manifest must keep strict backend gate pending before BFF alignment')
...
assert(manifest.frontendHandoffGates?.handoffRule?.includes('strict mode turns green only after resources BFF/page APIs are wired'), 'Panda handoff gates must explain strict-mode ownership')
...
assert(alignmentReportJson.apiWiredCount === 1, 'Panda JSON report must show one API-wired route before backend alignment')
assert(alignmentReportJson.mockReadyCount === pageIds.length - 1, 'Panda JSON report must show remaining routes as mock-ready before backend alignment')
...
assert(alignmentReportJson.resourcesBff?.defaultValue === 'false', 'Panda JSON report must keep resources BFF disabled by default')
...
assert(alignmentReportJson.deliveryReadiness?.strictBackendGate === 'pending-backend', 'Panda JSON report must keep strict backend gate pending')
assert(alignmentReportJson.frontendCloseout?.currentPhase === 'non-blocking-frontend-closeout', 'Panda JSON report must expose frontend closeout phase')
...
assert(alignmentReportJson.backendAlignmentBlockers?.status === 'pending-backend', 'Panda JSON report must classify strict blockers as pending-backend')
assert(alignmentReportJson.backendAlignmentBlockers?.pendingRoutes?.length === pageIds.length - 1, 'Panda JSON report must list mock-ready routes as backend alignment blockers')
assert(alignmentReportJson.backendAlignmentBlockers?.pendingTasks?.some((task) => task.id === 'resource-bff-dry-run'), 'Panda JSON report must expose the resources BFF dry-run blocker')
...
let strictFailedAsExpected = false
try {
  runNodeJson('scripts/panda-alignment-report.mjs', ['--json', '--strict'])
} catch (error) {
  strictFailedAsExpected = true
  const strictOutput = JSON.parse(error.stdout)
  assert(strictOutput.strict?.enabled === true, 'Panda strict JSON report must expose strict mode')
  assert(strictOutput.strict?.passed === false, 'Panda strict JSON report must fail while routes are mock-ready')
  assert(
    strictOutput.strict?.failures?.some((item) => item.includes('mock-ready')),
    'Panda strict JSON report must explain mock-ready route blockers',
  )
  ...
  assert(strictOutput.backendAlignmentBlockers?.status === 'pending-backend', 'Panda strict JSON report must keep backend blockers explicitly pending-backend')
}
assert(strictFailedAsExpected, 'Panda strict report must fail until backend resources are API-wired')
...
assert(resourceContractJson.keys?.readinessPendingRoutes?.includes('threads'), 'Panda resource contract probe must expose readiness pending routes')
...
assert(homeWorkbenchHook.includes('本地演示数据已接管'), 'usePandaHomeWorkbench must preserve mock fallback messaging')
...
assert(shell.includes('getWorkbenchConnectionLabel'), 'Panda shell must centralize the workbench connection label')
...
assert(shell.includes('const connectionLabel = getWorkbenchConnectionLabel'), 'PandaShellFrame must compute the connection label once')
...
assert(shell.includes('已连接 X-Agent Core'), 'Panda TopBar must preserve the X-Agent Core connection label')
...
assert(client.includes('mockWorkbenchHome'), 'workbenchClient must provide mock fallback')
assert(client.includes("source: 'mock'"), 'workbenchClient must mark mock fallback source')
assert(client.includes("from '../data/mockHome'"), 'workbenchClient must read the home fallback from mockHome')
...
assert(resourcesClient.includes("source: 'api'"), 'resourcesClient must mark API resource source')
assert(resourcesClient.includes("source: 'mock'"), 'resourcesClient must keep mock fallback source')
...
assert(envExample.includes('VITE_PANDA_RESOURCES_BFF=false'), 'Panda resources BFF flag must default to false in .env.example')
...
for (const pageId of pageIds) {
  assert(pageResourceContracts.includes(`${pageId}: {`), `Missing resource contract for page: ${pageId}`)
  assert(pageResourceContracts.includes(`page: '${pageId}'`), `Resource contract page mismatch for: ${pageId}`)
}
for (const runtimeField of pandaCoreRuntimeFieldNames) {
  assert(resourceRuntimeFields.includes(`'${runtimeField}'`) || pageResourceContracts.includes(`'${runtimeField}'`), `Resource contracts missing runtime field: ${runtimeField}`)
}
for (const resourceKey of pandaResourceSnapshotViewKeys) {
  ...
  assert(pageResourceContracts.includes(`'${resourceKey}'`), `No page resource contract references: ${resourceKey}`)
}
for (const endpoint of pandaPageBffEndpoints) {
  assert(pageResourceContracts.includes(endpoint), `Missing Panda BFF endpoint contract: ${endpoint}`)
}
...
assert(closeoutPlanJson.routeRolloverPlan.length === 11, 'Panda closeout plan must track the 11 pending backend route rollovers')
...
for (const typeName of pandaResourceViewModelTypeNames) {
  assert(resourceTypes.includes(`type ${typeName}`), `Missing Panda resource view model type: ${typeName}`)
}
assert(resourceTypes.includes('permissions: readonly string[]'), 'AgentProfile permissions must be readonly display data')
assert(resourceTypes.includes('evidenceRefs: readonly string[]'), 'AuditEvent evidenceRefs must be readonly display data')
...
assert(homeStatusSections.includes('Powered by X-Agent Autonomous Framework'), 'PlatformSnapshot must preserve the X-Agent core subtitle')
...
assert(threadExecutionWorkspace.includes('ActionPanel title="执行控制"'), 'threadExecutionWorkspace must render execution control actions')
assert(threadExecutionWorkspace.includes("from '../data/threadExecutionContent'"), 'threadExecutionWorkspace must load thread execution display fixtures from the focused data module')
for (const symbol of ['threadExecutionTabs', 'threadExecutionSteps', 'threadExecutionTerminalLines', 'threadExecutionControlActions', 'threadExecutionArtifactActions']) {
  ...
  assert(threadExecutionWorkspace.includes(symbol), `threadExecutionWorkspace must consume ${symbol} from threadExecutionContent`)
}
...
assert(taskQueue.includes('ActionPanel title="执行动作"'), 'taskQueue must render task execution actions')
...
assert(automationRulesPanel.includes('runtime={rule.runtime}'), 'automationRules must pass runtime metadata into ManagementRow')
...
assert(toolCenter.includes('工具发现由 MCP 管理器提供'), 'toolCenter must render the MCP discovery access boundary')
assert(toolCenter.includes('权限与审批由后端策略返回'), 'toolCenter must render the backend policy access boundary')
assert(toolCenter.includes('前端只展示状态、证据和可用动作'), 'toolCenter must keep frontend policy ownership explicit')
...
assert(settingsCenter.includes('X-Agent Core'), 'settingsCenter must preserve the technical core policy label')
...
for (const symbol of ['AgentRolePresetCard', 'AgentRolePresetDetail']) {
  assert(agentRolePresetCards.includes(`export function ${symbol}`), `agentRolePresetCards must own focused role-card component: ${symbol}`)
}
...
assert(agentRolePresetCards.includes('alt={`${preset.name}角色形象`}'), 'agent role portraits must expose role-specific alt text')
assert(agentRolePresetCards.includes('MiniTagList'), 'agent role preset detail must render abilities, tools, and permission tags')
assert(agentOrganization.includes('ActionPanel title="团队动作"'), 'agentOrganization must render team action controls')
...
assert(homeProjectSections.includes('loadingTitle="正在同步最近项目"'), 'home page recent projects must expose a module-specific loading state')
assert(homeProjectSections.includes('emptyTitle="暂无最近项目"'), 'home page recent projects must expose a module-specific empty state')
...
assert(rightRail.includes('runtime: agent.runtime'), 'RightRail agent activity fallback must preserve agent runtime metadata')
...
assert(rightRailStatusCards.includes('StatusDot'), 'RightRail system status card must use the shared status dot')
...
assert(rightRailWorkflowCard.includes('ariaLabel={`${workflow.name} 工作流进度`}'), 'RightRail workflow progress must expose a workflow-specific accessible label')
assert(rightRailResourceCard.includes('Resources BFF'), 'RightRail resource snapshot must show the resources BFF flag state')
assert(rightRailResourceCard.includes('BFF Endpoint'), 'RightRail resource snapshot must show the resources BFF endpoint')
assert(rightRailResourceCard.includes('resourcesBffConfig.enabled ?'), 'RightRail resource card must render the resources BFF enabled state from shared config')
assert(rightRailResourceCard.includes('resourcesBffConfig.endpoint'), 'RightRail resource card must render the resources BFF endpoint from shared config')
assert(rightRailStatusCards.includes('待审批变更'), 'RightRail status cards must render approval summary')
assert(rightRailStatusCards.includes('服务状态正常'), 'RightRail status cards must render system health status')
...
assert(css.includes('grid-template-columns: repeat(auto-fit, minmax(176px, 1fr))'), 'Panda role cards must auto-fit instead of forcing a clipped five-column grid')
