...
The plan is derived through `frontend/scripts/panda-alignment-context.mjs`, which reads `frontend/src/panda/pandaFrontendManifest.json`, `frontend/src/panda/pageResourceContractCatalog.ts`, `frontend/src/panda/pageResourceContracts.ts`, and `frontend/src/panda/api/resourceKeys.ts`, so it should not drift from the real route/resource contract ledger. Use it before front/back alignment to see:
...
- Shell layout composition lives in `frontend/src/panda/components/Shell.tsx`; shell sidebar and navigation live in `frontend/src/panda/components/shellChrome.tsx`, topbar search/status/mobile connection chrome lives in `frontend/src/panda/components/shellTopbar.tsx`, brand lockup lives in `frontend/src/panda/components/shellBranding.tsx`, workspace switching controls live in `frontend/src/panda/components/shellControls.tsx`, topbar command/user controls live in `frontend/src/panda/components/shellActionControls.tsx`, and workbench connection-state labels live in `frontend/src/panda/components/shellConnectionViewModel.ts`. Focused shell parts are re-exported through `shellChrome.tsx`, `shellBranding.tsx`, `shellControls.tsx`, and `Shell.tsx` for compatibility.
...
- Shared frontend types are exported through `frontend/src/panda/types.ts`. Focused type ownership lives in `frontend/src/panda/types/routeTypes.ts`, `frontend/src/panda/types/runtimeTypes.ts`, `frontend/src/panda/types/agentRoleTypes.ts`, and `frontend/src/panda/types/workbenchTypes.ts`. Panda resource view models are split by backend alignment surface across `frontend/src/panda/types/executionResourceTypes.ts`, `frontend/src/panda/types/organizationResourceTypes.ts`, `frontend/src/panda/types/knowledgeResourceTypes.ts`, and `frontend/src/panda/types/governanceResourceTypes.ts`, while `frontend/src/panda/types/resourceTypes.ts` remains the compatibility barrel.
...
- Panda-specific home data loading is split by responsibility: `frontend/src/panda/api/workbenchClient.ts` owns the current home API call and adapter invocation, while `frontend/src/panda/api/workbenchHomeLoadResult.ts` owns API/mock/error home result construction and fallback error copy.
...
- Page resource snapshot ownership is split by responsibility: `frontend/src/panda/api/resourceSnapshotTypes.ts` owns `PandaResourceSnapshot`, `PandaResourceLoadResult`, and `PandaResourceSource`; `frontend/src/panda/api/resourceFallbackSnapshot.ts` owns the mock fallback snapshot; `frontend/src/panda/api/resourcesApiLoader.ts` owns BFF loader injection, validation, and API-to-view mapping; `frontend/src/panda/api/resourcesLoadResult.ts` owns API success, mock fallback, degraded mock-with-error result construction, and resource-load error normalization.
- Page workspace state is split by responsibility: `frontend/src/panda/state/workspaceTypes.ts` owns workspace state types; `frontend/src/panda/state/workspaceProvider.tsx` owns provider runtime, context wiring, stale-response guards, and React state writes; `frontend/src/panda/state/workspaceLifecycleViewModel.ts` owns refresh time formatting, resource-load result mapping, status derivation, and refresh error normalization; `frontend/src/panda/state/workspaceHooks.ts` owns selectors and lifecycle hooks. `frontend/src/panda/state/PandaWorkspaceContext.tsx` remains the page-facing compatibility entrypoint.
...
- App-level state is split from the entry component: `frontend/src/panda/state/usePandaHashRoute.ts` owns hash routing and navigation, `frontend/src/panda/state/usePandaHomeWorkbench.ts` owns the home BFF/mock fallback lifecycle, and `frontend/src/panda/state/homeWorkbenchViewModel.ts` owns home workbench result-to-state mapping such as mock fallback error copy.
...
- Page-to-resource contract catalog entries live in `frontend/src/panda/pageResourceContractCatalog.ts`; this file maps each first-level module to its current resource slices, future BFF endpoint, and backend data needs. `frontend/src/panda/pageResourceContracts.ts` remains the compatibility/derived-key entrypoint, and `frontend/src/panda/resourceContracts.ts` remains a compatibility barrel.
...
- Route readiness handoff metadata lives in `frontend/src/panda/api/resourceReadiness.ts`; it derives from `pageResourceContracts.ts`, which re-exports the catalog from `pageResourceContractCatalog.ts` and exposes `pandaRouteReadiness` plus `pandaBackendAlignmentReadiness` for reports and backend alignment. In route readiness rows, `resources` are Panda view keys and `apiResources` are backend API keys derived through `frontend/src/panda/api/resourceKeys.ts`.
...
- `npm run verify:panda:components` runs the focused component primitive ownership probe for `common.tsx`, `pageChromePrimitives.tsx`, `pageContractPrimitives.tsx`, `pageContractViewModel.ts`, `statePanelBasePrimitives.tsx`, `statePanelPrimitives.tsx`, `resourceState.tsx`, `modulePageActionPrimitives.tsx`, `modulePagePrimitives.tsx`, `progressPrimitives.tsx`, `metricPrimitives.tsx`, `runtimeMetaPrimitives.tsx`, `tagListPrimitives.tsx`, `statusDotViewModel.ts`, `statusPrimitives.tsx`, `runtimePrimitives.tsx`, `workspacePrimitives.tsx`, `workspaceCardPrimitives.tsx`, `workspaceResourceCardPrimitives.tsx`, `workspaceListCardHeaderPrimitives.tsx`, `workspaceActivityPrimitives.tsx`, `workspaceRailPrimitives.tsx`, `workspaceLayoutPrimitives.tsx`, `workflowPrimitives.tsx`, `workflowEvidencePrimitives.tsx`, `workflowExecutionStepPrimitives.tsx`, and `workflowActionPrimitives.tsx`; `npm run verify:panda` also calls this probe.
...
- Panda-specific loading, empty, and degraded-data display primitives are split by responsibility: the shared panel shell lives in `frontend/src/panda/components/statePanelBasePrimitives.tsx`, while `frontend/src/panda/components/statePanelPrimitives.tsx` owns `PandaLoadingState`, `PandaEmptyState`, and `PandaErrorState` and re-exports the base shell for compatibility. Resource lifecycle handling stays in `frontend/src/panda/components/resourceState.tsx` through `PandaResourceState`, while `frontend/src/panda/components/resourceStateViewModel.ts` owns the loading/error/empty/ready branching and degraded-resource fallback copy from `usePandaWorkspaceLifecycle()`. Page headings and command buttons live in `frontend/src/panda/components/pageChromePrimitives.tsx`; visible page resource contracts live in `frontend/src/panda/components/pageContractPrimitives.tsx`, while `frontend/src/panda/components/pageContractViewModel.ts` owns readiness/source/field/refresh label mapping for the strip. These are re-exported through `common.tsx` for compatibility.
...
- Right rail dynamic cards are split by responsibility: agent activity lives in `frontend/src/panda/components/rightRailActivityCard.tsx`, while `frontend/src/panda/components/rightRailActivityCardViewModel.ts` owns activity card header copy, empty-state copy, and `ActivityItem` row display mapping; workflow progress lives in `frontend/src/panda/components/rightRailWorkflowCard.tsx`, workflow run labels and progress aria text live in `frontend/src/panda/components/rightRailWorkflowCardViewModel.ts`, the resource snapshot/BFF status card lives in `frontend/src/panda/components/rightRailResourceCard.tsx`, static approval/risk plus system status card shells live in `frontend/src/panda/components/rightRailStatusCards.tsx`, their static card titles and row view models live in `frontend/src/panda/components/rightRailStatusCardsViewModel.tsx`, and home-or-resource fallback mapping lives in `frontend/src/panda/components/rightRailFallbacks.ts`. Agent activity rows render shared runtime metadata tags through `RuntimeMetaStrip`; right rail key-value status rows render through shared `KeyValueList` instead of duplicating row layout. Focused cards are re-exported through `rightRailCards.tsx` for compatibility.
- Home task input lives in `frontend/src/panda/components/homeTaskComposer.tsx`; home prompt actions live in `frontend/src/panda/components/homeActionSections.tsx`; home quick/module navigation grids live in `frontend/src/panda/components/homeNavigationSections.tsx`; recent project resources render in `frontend/src/panda/components/homeProjectSections.tsx`, while `frontend/src/panda/components/homeProjectSectionsViewModel.ts` owns recent project header copy, loading/empty copy, table columns, and project row display mapping. Home BFF status and X-Agent core summary render in `frontend/src/panda/components/homeStatusSections.tsx`, while `frontend/src/panda/components/homeStatusSectionsViewModel.ts` owns metric row mapping, mock fallback units, loading copy, degraded fallback copy, and summary fallback copy. Home quick/module navigation grids compose shared `NavigationCardGrid`, and module summary card bodies compose shared `ModuleSummaryCard`. All are re-exported through `homeSections.tsx` for compatibility, with `homeActionSections.tsx` preserving compatibility exports for the split navigation sections.
- Thread list and resource-state shell live in `frontend/src/panda/components/threadWorkspace.tsx`, while `frontend/src/panda/components/threadWorkspaceViewModel.ts` owns thread workspace header copy, new-thread accessible label, empty-state copy, and thread list row display mapping. The execution workspace renderer lives in `frontend/src/panda/components/threadExecutionWorkspace.tsx`, while `frontend/src/panda/components/threadExecutionWorkspaceViewModel.ts` maps execution tabs, plan steps, terminal output, completion state, evidence refs, execution controls, and artifact labels from `frontend/src/panda/data/threadExecutionContent.ts` so they can be replaced by backend execution payloads without changing layout code. The execution workspace is re-exported through `threadWorkspace.tsx` for compatibility.
- Agent organization overview lives in `frontend/src/panda/components/agentOrganization.tsx`; it owns the team topology panel and agent grid composition, while overview header copy, team action panel data, team action rows, lead-agent control panel data, and lead-agent control rows live in `frontend/src/panda/components/agentOrganizationViewModel.ts`, and `frontend/src/panda/components/agentProfileCards.tsx` owns individual agent runtime cards. Create-agent role card selection lives in `frontend/src/panda/components/agentRolePresetSelector.tsx`; it owns the role selector state and layout, while `frontend/src/panda/components/agentRolePresetCards.tsx` owns compact role cards and `frontend/src/panda/components/agentRolePresetDetail.tsx` owns the detail panel with the large role portrait, abilities, tools, and permission tags. Role cards render the user-reference character assets from `frontend/src/panda/data/agentRolePortraits.ts` through `portrait_key`, and both compact cards and detail panels are guarded from falling back to generic icon placeholders. Agent profile and role-card components are re-exported through `frontend/src/panda/components/agentOrganization.tsx` for compatibility.
- Progress primitives such as `ProgressMeter` and `ProgressSummary` live in `frontend/src/panda/components/progressPrimitives.tsx`; metric strips live in `frontend/src/panda/components/metricPrimitives.tsx`; runtime metadata tag rendering lives in `frontend/src/panda/components/runtimeMetaPrimitives.tsx`, while `frontend/src/panda/components/runtimeMetaViewModel.ts` owns the `owner_agent`, `updated_at`, `risk_level`, and `evidence_refs` display tag mapping from `RuntimeMetadata`; shared mini tags and key-value rows live in `frontend/src/panda/components/tagListPrimitives.tsx`; accessible status dots live in `frontend/src/panda/components/statusPrimitives.tsx`, while `frontend/src/panda/components/statusDotViewModel.ts` owns tone class, title, and aria-label mapping. `metricPrimitives.tsx` and `runtimePrimitives.tsx` remain compatibility barrels for existing imports.
- Workspace card primitives are split by responsibility: `frontend/src/panda/components/workspaceCardPrimitives.tsx` owns generic card grids and module summary bodies, `frontend/src/panda/components/workspaceResourceCardPrimitives.tsx` owns resource runtime and resource info card composition, `frontend/src/panda/components/workspaceListCardHeaderPrimitives.tsx` owns list-card headers and status dots, while `frontend/src/panda/components/workspaceCapabilityCardPrimitives.tsx` owns tool-card headers and `CapabilityMetricCard`; `frontend/src/panda/components/workspaceInfoPrimitives.tsx` owns `InfoPairGrid` and `InsetInfoBlock`; `frontend/src/panda/components/workspaceTablePrimitives.tsx` owns `WorkspaceTable` and `WorkspaceTableHeader`. `workspaceResourceCardPrimitives.tsx`, `workspaceCardPrimitives.tsx`, and `workspacePrimitives.tsx` re-export focused card/info/table primitives for compatibility. Activity rows live in `frontend/src/panda/components/workspaceActivityPrimitives.tsx`; right-rail card shells live in `frontend/src/panda/components/workspaceRailPrimitives.tsx`; workspace panels and section headers live in `frontend/src/panda/components/workspaceLayoutPrimitives.tsx`, which preserves the activity-row and rail-card compatibility exports. `WorkspacePanel` centralizes the common `panda-card p-4` workspace shell across home status, tasks, projects, audit, automation, and tool access boundary sections while preserving semantic `section`, `div`, or `aside` containers. `frontend/src/panda/components/workflowEvidencePrimitives.tsx` owns audit event rows, `frontend/src/panda/components/workflowExecutionStepPrimitives.tsx` owns execution timeline rows with `owner_agent` and `evidence_refs`, `frontend/src/panda/components/workflowNodePrimitives.tsx` owns workflow canvas node cards, and `frontend/src/panda/components/workflowActionPrimitives.tsx` owns action panels, `PanelActionButton`, and management rows; panel action buttons centralize contextual accessible labels for execution/control panels, and management rows render runtime evidence through `RuntimeMetaStrip` so tasks and automation keep the same owner, update, risk, and evidence tag semantics as other resource surfaces. `workspacePrimitives.tsx` and `workflowPrimitives.tsx` remain compatibility barrels re-exported through `common.tsx`.
...
- `frontend/src/panda/pageResourceContractCatalog.ts` is the frontend-owned BFF alignment ledger while backend mainline closes; `frontend/src/panda/pageResourceContracts.ts` remains the compatibility/derived-key entrypoint.
...
- `frontend/src/panda/api/resourceReadiness.ts` derives a route-level readiness list from `pageResourceContracts.ts`, backed by `pageResourceContractCatalog.ts`; backend alignment should use `pandaRouteReadiness` for route rollout order, `apiResources` for backend resource names, and `pandaBackendAlignmentReadiness` for strict-gate requirements.
...
- `frontend/src/panda/api/workbenchClient.ts` owns the current home API call and adapter mapping. `frontend/src/panda/api/workbenchHomeLoadResult.ts` owns home API success, mock fallback, and fallback error result construction. `PandaAgentApp` should call this client instead of importing global API methods directly.
- `frontend/src/panda/api/resourcesClient.ts` owns the compatibility loading contract for non-home pages. It currently returns mock data through `loadPandaResources()` and delegates focused responsibilities to the snapshot type, fallback snapshot, API loader, and resource load result builder modules.
...
- `setPandaResourcesApiLoader()` may be wired to a real BFF client after backend closeout through `resourcesApiLoader.ts`; failed API loads must keep returning `{ source: "mock", error }` from `loadPandaResources()` via `resourcesLoadResult.ts` so the UI labels degraded data instead of crashing.
...
- `frontend/src/panda/state/PandaWorkspaceContext.tsx` is the provider/resource compatibility entrypoint. Its focused implementation lives in `workspaceTypes.ts`, `workspaceProvider.tsx`, `workspaceLifecycleViewModel.ts`, and `workspaceHooks.ts`.
...
- The right rail, `ThreadsPage`, home resource sections, and selector helpers may use workspace hooks from `PandaWorkspaceContext.tsx` directly because they own custom layouts or cross-resource lifecycle state; right rail resource fallbacks should stay in `rightRailFallbacks.ts` instead of inline component mapping.
- `frontend/src/panda/components/workflowCanvasViewModel.ts` owns workflow canvas title/subtitle derivation, approval-gateway counts, empty workflow fallback copy, `runtime.evidenceRefs` summary text, and workflow run card title/description/progress/runtime display fields so `WorkflowCanvas` does not hard-code mock node counts or run-card formatting when backend workflow resources arrive.
- `frontend/src/panda/components/auditReplayViewModel.ts` owns audit risk summary derivation from `AuditEvent[]`, including approval-change counts, danger-risk counts, replay count, and evidence reference totals, so audit summary cards stay aligned with API-backed audit events instead of static demo numbers.
- `frontend/src/panda/components/taskQueueViewModel.ts` owns the task queue section title, task row titles, tones, descriptions, runtime metadata, progress values, progress labels, execution action labels, and the execution action panel title so `TaskQueueWorkspace` remains a display component over `TaskSummary[]` instead of embedding task formatting rules in JSX.
- `frontend/src/panda/components/automationRulesViewModel.ts` owns the automation section title, automation rule row titles, tones, descriptions, runtime metadata, status text, and last-run display values so `AutomationRulesPanel` stays a display component over `AutomationRule[]` while backend automation fields are finalized.
- `frontend/src/panda/components/knowledgeBaseViewModel.ts` owns knowledge source card titles, descriptions, and info pairs from `KnowledgeSource[]`, keeping knowledge-page JSX focused on rendering while API-backed source metadata is finalized.
- `frontend/src/panda/components/dataCenterViewModel.ts` owns data source card titles, subtitles, and metric rows from `DataSource[]`, keeping data-center JSX focused on rendering while API-backed source status, record, and sync fields are finalized.
- `frontend/src/panda/components/toolCenterViewModel.ts` owns tool capability titles, subtitles, metric rows, tool access boundary title, and tool access boundary copy, keeping tool-center JSX focused on rendering while MCP discovery, permissions, and policy fields are finalized.
- `frontend/src/panda/components/settingsCenterViewModel.ts` owns settings section titles, descriptions, status/policy info pairs, and the `X-Agent Core` technical-core label, keeping settings-center JSX focused on rendering while brand/API metadata is finalized.
- `frontend/src/panda/components/agentProfileCardViewModel.ts` owns agent profile card titles, descriptions, progress values, permission tags, and runtime owner/risk display values from `AgentProfile[]`, keeping agent-card JSX focused on shared runtime, progress, and tag primitives.
- `frontend/src/panda/components/projectWorkspaceViewModel.ts` owns the project workspace section title, project table columns, and runtime owner/updatedAt/risk display values from `ProjectItem[]`, keeping project workspace JSX focused on table and runtime strip rendering.
- `frontend/src/panda/components/agentRolePresetViewModel.ts` owns create-agent role card portrait alt text and role detail block labels/items from `AgentRolePreset`, keeping role-card JSX focused on rendering portraits and tag lists while backend role templates are pending.
