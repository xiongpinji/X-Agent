...
| B2-P1-04 | Second-batch runtime capability manifest | Implemented and locally verified | Mounted `/api/v1/capabilities/second-batch` read-only manifest with `audit:read`, capability IDs, API surfaces, required scopes, evidence types, API-first/no-local-model boundary, owner-gated live-use flags, and no release claim | `python -m pytest tests/test_second_batch_capabilities_api.py tests/test_second_batch_capability_manifest_gate.py -q --no-cov`; `python scripts/second_batch_capability_manifest_gate.py`; `python scripts/second_batch_quality_gate.py`; `git diff --check` |
...
- `/api/v1/capabilities/second-batch` is a read-only visibility surface. It does not execute providers, spawn agents, launch browsers, or promote owner-gated capabilities to production readiness.
