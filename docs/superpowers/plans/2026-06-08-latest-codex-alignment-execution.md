...
### Task 38: Runtime Flag Application Preflight Workflow

**Target:** Record the owner-gated preflight evidence required before live runtime flag application, while still keeping `XAGENT_SDK_WRITE_RUNNER_ENABLED` disabled and the SDK write-runner uninvoked.
...
- [x] Add `/api/v1/control-plane/sdk/runtime-flag/application-preflight/record` as an owner-gated runtime flag application preflight endpoint.
- [x] Add SDK and non-interactive CLI wrappers for runtime flag application preflight recording.
- [x] Require approved SDK approval, recorded runtime flag enablement intent, runtime flag name, target state, rollback plan, smoke runbook, timestamp, reason, signature/hash, and dry-run mode before recording.
- [x] Keep preflight recording read-only with respect to runtime effects: it does not apply `XAGENT_SDK_WRITE_RUNNER_ENABLED`, invoke the write runner, mark approvals executed, or perform file/network/channel mutation.
...
- [x] Define the live runtime flag application execute contract only after owner approval review; keep write-runner wiring as a separate follow-up gate.

### Task 40: Live Runtime Flag Application Execute Contract Workflow

**Target:** Record the owner-gated execute contract required before live runtime flag application, while still keeping `XAGENT_SDK_WRITE_RUNNER_ENABLED` disabled and the SDK write-runner uninvoked.
...
- [x] Add `/api/v1/control-plane/sdk/runtime-flag/application-execute-contract/record` as an owner-gated execute contract endpoint.
- [x] Add SDK and non-interactive CLI wrappers for runtime flag application execute contract recording.
...
### Task 43: Live Runtime Flag Application Readiness Plan Decision Workflow

**Target:** Record the owner accept/reject decision for the live runtime flag application readiness plan, while still keeping `XAGENT_SDK_WRITE_RUNNER_ENABLED`, the future adapter, and the SDK write-runner disabled.
...
- [x] Add `/api/v1/control-plane/sdk/runtime-flag/application-readiness-plan/decision/record` as an owner-gated readiness plan decision endpoint.
- [x] Add SDK wrapper, HTTP client adapter, and non-interactive CLI command `xagent sdk runtime-flag-application-readiness-plan-decision-record --execute`.
...
- [x] Defer live adapter implementation until a separate owner design review workflow records acceptance.

### Task 45: Live Runtime Flag Application Adapter Design Review Workflow

**Target:** Record owner design review for the future live runtime flag application adapter after an accepted implementation request, while still keeping `XAGENT_SDK_WRITE_RUNNER_ENABLED`, adapter import/instantiation, adapter execution, and SDK write-runner execution disabled.
...
- [x] Add `/api/v1/control-plane/sdk/runtime-flag/application-adapter/design-review/record` as an owner-gated adapter design review endpoint.
- [x] Add SDK wrapper, HTTP client adapter, and non-interactive CLI command `xagent sdk runtime-flag-application-adapter-design-review-record --execute`.
- [x] Require approved SDK approval, recorded accepted adapter implementation request audit, runtime flag name, accept/reject review decision, adapter design reference, security review reference, test plan reference, rollback plan reference, smoke runbook reference, timestamp, reason, signature/hash, and dry-run mode before recording.
- [x] Keep adapter design review recording read-only with respect to runtime effects: it does not read, write, or apply `XAGENT_SDK_WRITE_RUNNER_ENABLED`, import/instantiate/enable the adapter, invoke the write runner, mark approvals executed, or perform file/network/channel mutation.
- [x] Add `/sdk/invoke` adapter design review workflow metadata and promote SDK alignment status to `sdk_live_runtime_flag_application_adapter_design_review_workflow_ready`.
- [x] Defer live adapter code changes until a separate owner-gated implementation preflight records acceptance.

### Task 46: Live Runtime Flag Application Adapter Implementation Preflight Workflow

**Target:** Record owner-gated implementation preflight for the future live runtime flag application adapter after accepted design review, while still keeping `XAGENT_SDK_WRITE_RUNNER_ENABLED`, adapter import/instantiation, adapter execution, SDK write-runner execution, and adapter file writes disabled.
...
- [x] Add `/api/v1/control-plane/sdk/runtime-flag/application-adapter/implementation-preflight/record` as an owner-gated adapter implementation preflight endpoint.
- [x] Add SDK wrapper, HTTP client adapter, and non-interactive CLI command `xagent sdk runtime-flag-application-adapter-implementation-preflight-record --execute`.
- [x] Require approved SDK approval, recorded accepted adapter design review audit, runtime flag name, implementation branch reference, implementation plan reference, adapter design reference, security review reference, test plan reference, rollback plan reference, smoke runbook reference, idempotency key/hash, signature/hash, and dry-run mode before recording.
- [x] Keep adapter implementation preflight recording read-only with respect to runtime effects: it does not read, write, or apply `XAGENT_SDK_WRITE_RUNNER_ENABLED`, import/instantiate/enable the adapter, write adapter files, invoke the write runner, mark approvals executed, or perform file/network/channel mutation.
- [x] Add `/sdk/invoke` adapter implementation preflight workflow metadata and promote SDK alignment status to `sdk_live_runtime_flag_application_adapter_implementation_preflight_workflow_ready`.
- [x] Implement the disabled live runtime flag application adapter code-change gate after accepted implementation preflight is recorded; keep write-runner wiring as a separate follow-up gate.

### Task 47: Live Runtime Flag Application Adapter Code Change Workflow

**Target:** Add the disabled live runtime flag application adapter module and record the owner-gated code-change gate after accepted implementation preflight, while still keeping `/sdk/invoke` unwired from the adapter and preserving disabled runtime flag application and write-runner execution.

**Files:**
- Add: `backend/app/sdk/runtime_flag_application_adapter.py`
- Modify: `backend/app/api/control_plane.py`
- Modify: `backend/app/sdk/control_plane.py`
- Modify: `backend/app/sdk/__init__.py`
- Modify: `cli/client.py`
...
- Modify: `tests/test_sdk_noninteractive_report.py`
- Modify: `tests/test_latest_codex_alignment.py`
- Add: `tests/test_sdk_runtime_flag_application_adapter.py`
- Runtime evidence: `.xagent_runtime/reports/sdk-noninteractive-report.json`
- Runtime evidence: `.xagent_runtime/reports/latest-codex-alignment.json`

- [x] Add `backend.app.sdk.runtime_flag_application_adapter.SDKRuntimeFlagApplicationAdapter` as a disabled dry-run plan builder only.
- [x] Add `/api/v1/control-plane/sdk/runtime-flag/application-adapter/code-change/record` as an owner-gated adapter code-change gate endpoint.
- [x] Add SDK wrapper, HTTP client adapter, and non-interactive CLI command `xagent sdk runtime-flag-application-adapter-code-change-record --execute`.
...
- [x] Add a separate owner-approved runtime preflight gate for the disabled live runtime flag application adapter; keep write-runner invocation as a later follow-up.

### Task 49: Live Runtime Flag Application Adapter Runtime Preflight Workflow

**Target:** Record the owner-approved runtime preflight intent for the disabled live runtime flag application adapter after accepted adapter wiring evidence, while still keeping `/sdk/invoke` unwired from the adapter and preserving disabled runtime flag application and write-runner execution.
...
- [x] Add `/api/v1/control-plane/sdk/runtime-flag/application-adapter/runtime-preflight/record` as an owner-gated adapter runtime preflight endpoint.
- [x] Add SDK wrapper, HTTP client adapter, and non-interactive CLI command `xagent sdk runtime-flag-application-adapter-runtime-preflight-record --execute`.
- [x] Require approved SDK approval, recorded accepted adapter wiring audit, runtime flag name, adapter module/class, runtime preflight plan reference, wiring plan reference, implementation branch/plan refs, security/test/rollback/smoke refs, idempotency key/hash, signature/hash, and dry-run mode before recording.
- [x] Keep adapter runtime preflight recording read-only with respect to runtime effects: it does not wire `/sdk/invoke`, import/instantiate/execute the adapter in runtime path, read/write/apply `XAGENT_SDK_WRITE_RUNNER_ENABLED`, invoke the write runner, mark approvals executed, or perform file/network/channel mutation.
- [x] Add `/sdk/invoke` adapter runtime preflight workflow metadata and promote SDK alignment status to `sdk_live_runtime_flag_application_adapter_runtime_preflight_workflow_ready`.
- [x] Add a separate owner-approved execution dry-run gate for the disabled live runtime flag application adapter; keep write-runner invocation as a later follow-up.

### Task 50: Live Runtime Flag Application Adapter Execution Dry-Run Workflow

**Target:** Record the owner-approved execution dry-run intent for the disabled live runtime flag application adapter after accepted adapter runtime preflight evidence, while still keeping `/sdk/invoke` unwired from the adapter and preserving disabled runtime flag application and write-runner execution.
...
- [x] Add `/api/v1/control-plane/sdk/runtime-flag/application-adapter/execution-dry-run/record` as an owner-gated adapter execution dry-run endpoint.
- [x] Add SDK wrapper, HTTP client adapter, and non-interactive CLI command `xagent sdk runtime-flag-application-adapter-execution-dry-run-record --execute`.
- [x] Require approved SDK approval, recorded accepted adapter runtime preflight audit, runtime flag name, adapter module/class, execution dry-run plan reference, runtime preflight plan reference, wiring plan reference, implementation branch/plan refs, security/test/rollback/smoke refs, idempotency key/hash, signature/hash, and dry-run mode before recording.
- [x] Keep adapter execution dry-run recording read-only with respect to runtime effects: it does not wire `/sdk/invoke`, import/instantiate/execute the adapter in runtime path, read/write/apply `XAGENT_SDK_WRITE_RUNNER_ENABLED`, invoke the write runner, mark approvals executed, or perform file/network/channel mutation.
- [x] Add `/sdk/invoke` adapter execution dry-run workflow metadata and promote SDK alignment status to `sdk_live_runtime_flag_application_adapter_execution_dry_run_workflow_ready`.
- [ ] Add a separate owner-approved live adapter execution gate after dry-run evidence review; keep write-runner invocation disabled until owner acceptance.

## Completion Criteria

- `scripts/latest_codex_alignment.py` returns `latest_codex_alignment_plan_ready`.
- `scripts/sdk_noninteractive_report.py` returns `sdk_live_runtime_flag_application_adapter_execution_dry_run_workflow_ready`.
