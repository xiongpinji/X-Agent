...
> ## ⚠️ 架构决策公告（P1-11，2026-07-20）
>
> **本文档描述的 `skills_*` 扁平栈（skills_core/loader/registry/executor/sandbox/
> marketplace/manager/cli/document）已被指定为 LEGACY 管理平面，不再是技能运行时。**
>
> 历史上仓库存在三套技能实现，P1-11 选定唯一运行时如下：
>
> | 实现 | 位置 | 决策 |
> |---|---|---|
> | **目录技能运行时（选定）** | `backend/app/core/skills/`（skill_base/skill_loader/skill_registry） | ✅ 唯一运行时 |
> | skills_* 扁平栈（本文档） | `backend/app/core/skills_*.py` | LEGACY 管理平面，仅保留兼容测试与 `skills_api.py` |
> | skill_system_v2 | `backend/app/core/skill_system_v2.py` | LEGACY，仅供 `skill_chain` 使用 |
>
> **唯一运行时的关键事实（以代码实际为准）：**
> - 技能目录：项目根 `skills/` 与 `custom-skills/`（`SkillLoader` 默认指向，
>   见 `backend/app/core/skills/skill_loader.py::get_default_skills_dirs`）。
> - 技能形态：`skills/<name>/SKILL.md + main.py`（或 `skill.py`），
>   `main.py` 导出 `SkillImplementation` 类（继承 `backend.app.core.skills.Skill`）。
> - 现有可执行技能：`code-review-skill`（AST 静态审查，仅 Python）、
>   `data-analysis-skill`（CSV 统计画像，无图表/预测）。
> - Agent 消费：`backend/app/core/skill_agent_adapter.py::register_skills_into_tool_registry`
>   把技能注册为 AgentLoop ToolRegistry 的 `skill__<name>` 工具（集成波接线，见
>   `skills/RUNTIME_INTEGRATION.md`）。
> - 加载结果显式可追溯：`SkillLoader.load_report` 记录每个技能成败原因，不静默跳过。
