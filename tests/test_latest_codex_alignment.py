...
def _sdk_noninteractive_payload(
    *,
    status: str = "sdk_live_runtime_flag_application_adapter_execution_gate_workflow_ready",
    parity: bool = False,
    mutation: bool = False,
) -> dict[str, object]:
    return {
...
def _evidence_specs(paths: dict[str, Path]) -> tuple[AlignmentEvidenceSpec, ...]:
    return (
        ...
        AlignmentEvidenceSpec(
            ...
            paths["sdk_report"],
            "runtime_report",
            expected_statuses=frozenset({"sdk_live_runtime_flag_application_adapter_execution_gate_workflow_ready"}),
...
def test_latest_codex_alignment_ready_with_current_evidence(tmp_path: Path) -> None:
    ...
    assert sdk.xagent_status == "sdk_live_runtime_flag_application_adapter_execution_gate_workflow_ready"
    assert {
        "sdk_noninteractive_report",
        "sdk_contract_module",
        "sdk_cli_command",
