...
def _sdk_noninteractive_payload(
    *,
    status: str = "sdk_live_runtime_flag_application_implementation_readiness_plan_ready",
    parity: bool = False,
    mutation: bool = False,
) -> dict[str, object]:
    return {
...
def test_latest_codex_alignment_ready_with_current_evidence(tmp_path: Path) -> None:
    ...
    assert sdk.xagent_status == "sdk_live_runtime_flag_application_implementation_readiness_plan_ready"
    assert {
        "sdk_noninteractive_report",
        "sdk_contract_module",
        "sdk_cli_command",
