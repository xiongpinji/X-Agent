...
def test_sdk_runtime_flag_application_adapter_code_change_is_safe() -> None:
    payload = ControlPlaneSDK().record_runtime_flag_application_adapter_code_change(
    ...
    assert payload["operation"] == "runtime_flag_application_adapter_code_change_record"
    assert payload["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/code-change/record"
    )
    assert payload["request"]["adapter_code_change_id"] == "adapter-code-change-1"
    assert payload["request"]["approval_id"] == "approval-1"
    assert payload["request"]["adapter_implementation_preflight_id"] == (
        "adapter-implementation-preflight-1"
    )
    assert payload["request"]["adapter_implementation_preflight_audit_id"] == (
        "audit-adapter-implementation-preflight-1"
    )
    assert payload["request"]["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["request"]["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["request"]["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["request"]["code_change_hash"] == "hash-adapter-code-change-1"
    assert payload["request"]["dry_run"] is True
    assert payload["owner_gate"]["requires_approved_sdk_approval"] is True
    assert payload["owner_gate"]["requires_accepted_adapter_implementation_preflight"] is True
    assert payload["owner_gate"]["requires_adapter_implementation_preflight_audit"] is True
    assert payload["owner_gate"]["requires_runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["owner_gate"]["requires_adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["owner_gate"]["requires_adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["owner_gate"]["requires_signature_or_hash"] is True
    assert payload["owner_gate"]["runtime_flag_enabled"] is False
    assert payload["owner_gate"]["flag_application_performed"] is False
    assert payload["owner_gate"]["implementation_enabled"] is False
...
def test_sdk_runtime_flag_application_adapter_wiring_is_safe() -> None:
    payload = ControlPlaneSDK().record_runtime_flag_application_adapter_wiring(
        adapter_wiring_id="adapter-wiring-1",
        approval_id="approval-1",
        adapter_code_change_id="adapter-code-change-1",
        adapter_code_change_audit_id="audit-adapter-code-change-1",
        adapter_implementation_preflight_id="adapter-implementation-preflight-1",
        adapter_design_review_id="adapter-design-review-1",
        adapter_implementation_request_id="adapter-implementation-request-1",
        readiness_plan_decision_id="readiness-plan-decision-1",
    ...
    assert payload["operation"] == "runtime_flag_application_adapter_wiring_record"
    assert payload["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/wiring/record"
    )
    assert payload["request"]["adapter_wiring_id"] == "adapter-wiring-1"
    assert payload["request"]["approval_id"] == "approval-1"
    assert payload["request"]["adapter_code_change_id"] == "adapter-code-change-1"
    assert payload["request"]["adapter_code_change_audit_id"] == "audit-adapter-code-change-1"
    assert payload["request"]["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["request"]["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["request"]["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["request"]["wiring_plan_ref"] == (
        "docs/runbooks/sdk-write-runner-runtime-flag-adapter-wiring.md"
    )
    assert payload["request"]["wiring_hash"] == "hash-adapter-wiring-1"
    assert payload["request"]["dry_run"] is True
    assert payload["owner_gate"]["requires_approved_sdk_approval"] is True
    assert payload["owner_gate"]["requires_accepted_adapter_code_change"] is True
    assert payload["owner_gate"]["requires_adapter_code_change_audit"] is True
    assert payload["owner_gate"]["requires_runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["owner_gate"]["requires_adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["owner_gate"]["requires_adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["owner_gate"]["requires_wiring_plan_ref"] is True
    assert payload["owner_gate"]["requires_signature_or_hash"] is True
    assert payload["owner_gate"]["runtime_flag_enabled"] is False
    assert payload["owner_gate"]["flag_application_performed"] is False
    assert payload["owner_gate"]["implementation_enabled"] is False
    ...
    assert payload["owner_gate"]["adapter_import_allowed"] is False
    assert payload["owner_gate"]["adapter_execution_allowed"] is False
    assert payload["owner_gate"]["wired_into_sdk_invoke"] is False
    assert payload["owner_gate"]["adapter_runtime_wired"] is False
    assert payload["owner_gate"]["imports_adapter_in_sdk_invoke"] is False
    assert payload["owner_gate"]["instantiates_adapter_in_sdk_invoke"] is False
    assert payload["mutation_performed"] is False
    assert payload["network_mutation_performed"] is False
    assert payload["file_mutation_performed"] is False
    assert payload["wired_into_sdk_invoke"] is False
    assert payload["adapter_runtime_wired"] is False


def test_sdk_runtime_flag_application_adapter_runtime_preflight_is_safe() -> None:
    payload = ControlPlaneSDK().record_runtime_flag_application_adapter_runtime_preflight(
        adapter_runtime_preflight_id="adapter-runtime-preflight-1",
        approval_id="approval-1",
        adapter_wiring_id="adapter-wiring-1",
        adapter_wiring_audit_id="audit-adapter-wiring-1",
        adapter_code_change_id="adapter-code-change-1",
        adapter_implementation_preflight_id="adapter-implementation-preflight-1",
        adapter_design_review_id="adapter-design-review-1",
        adapter_implementation_request_id="adapter-implementation-request-1",
    ...
    assert payload["operation"] == "runtime_flag_application_adapter_runtime_preflight_record"
    assert payload["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/runtime-preflight/record"
    )
    assert payload["request"]["adapter_runtime_preflight_id"] == "adapter-runtime-preflight-1"
    assert payload["request"]["approval_id"] == "approval-1"
    assert payload["request"]["adapter_wiring_id"] == "adapter-wiring-1"
    assert payload["request"]["adapter_wiring_audit_id"] == "audit-adapter-wiring-1"
    assert payload["request"]["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["request"]["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["request"]["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["request"]["runtime_preflight_plan_ref"] == (
        "docs/runbooks/sdk-write-runner-runtime-flag-adapter-runtime-preflight.md"
    )
    assert payload["request"]["runtime_preflight_hash"] == "hash-adapter-runtime-preflight-1"
    assert payload["request"]["dry_run"] is True
    assert payload["owner_gate"]["requires_approved_sdk_approval"] is True
    assert payload["owner_gate"]["requires_accepted_adapter_wiring"] is True
    assert payload["owner_gate"]["requires_adapter_wiring_audit"] is True
    assert payload["owner_gate"]["requires_runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["owner_gate"]["requires_adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["owner_gate"]["requires_adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["owner_gate"]["requires_runtime_preflight_plan_ref"] is True
    assert payload["owner_gate"]["requires_signature_or_hash"] is True
    assert payload["owner_gate"]["runtime_flag_enabled"] is False
    assert payload["owner_gate"]["flag_application_performed"] is False
    assert payload["owner_gate"]["implementation_enabled"] is False
...
def test_sdk_runtime_flag_application_adapter_execution_dry_run_is_safe() -> None:
    payload = ControlPlaneSDK().record_runtime_flag_application_adapter_execution_dry_run(
        adapter_execution_dry_run_id="adapter-execution-dry-run-1",
        approval_id="approval-1",
        adapter_runtime_preflight_id="adapter-runtime-preflight-1",
        adapter_runtime_preflight_audit_id="audit-adapter-runtime-preflight-1",
        adapter_wiring_id="adapter-wiring-1",
        adapter_code_change_id="adapter-code-change-1",
        adapter_implementation_preflight_id="adapter-implementation-preflight-1",
        adapter_design_review_id="adapter-design-review-1",
    ...
    assert payload["operation"] == "runtime_flag_application_adapter_execution_dry_run_record"
    assert payload["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/execution-dry-run/record"
    )
    assert payload["request"]["adapter_execution_dry_run_id"] == "adapter-execution-dry-run-1"
    assert payload["request"]["approval_id"] == "approval-1"
    assert payload["request"]["adapter_runtime_preflight_id"] == "adapter-runtime-preflight-1"
    assert payload["request"]["adapter_runtime_preflight_audit_id"] == "audit-adapter-runtime-preflight-1"
    assert payload["request"]["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["request"]["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["request"]["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["request"]["execution_dry_run_plan_ref"] == (
        "docs/runbooks/sdk-write-runner-runtime-flag-adapter-execution-dry-run.md"
    )
    assert payload["request"]["execution_dry_run_hash"] == "hash-adapter-execution-dry-run-1"
    assert payload["request"]["dry_run"] is True
    assert payload["owner_gate"]["requires_approved_sdk_approval"] is True
    assert payload["owner_gate"]["requires_accepted_adapter_runtime_preflight"] is True
    assert payload["owner_gate"]["requires_adapter_runtime_preflight_audit"] is True
    assert payload["owner_gate"]["requires_runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["owner_gate"]["requires_adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["owner_gate"]["requires_adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["owner_gate"]["requires_execution_dry_run_plan_ref"] is True
    assert payload["owner_gate"]["requires_signature_or_hash"] is True
    assert payload["owner_gate"]["runtime_flag_enabled"] is False
    assert payload["owner_gate"]["flag_application_performed"] is False
    assert payload["owner_gate"]["implementation_enabled"] is False
    ...
    assert payload["owner_gate"]["imports_adapter_in_sdk_invoke"] is False
    assert payload["owner_gate"]["instantiates_adapter_in_sdk_invoke"] is False
    assert payload["owner_gate"]["execution_dry_run_invoked"] is False
    assert payload["mutation_performed"] is False
    assert payload["network_mutation_performed"] is False
    assert payload["file_mutation_performed"] is False
    assert payload["wired_into_sdk_invoke"] is False
    assert payload["adapter_runtime_wired"] is False
    assert payload["execution_dry_run_invoked"] is False


def test_sdk_runtime_flag_application_adapter_execution_gate_is_safe() -> None:
    payload = ControlPlaneSDK().record_runtime_flag_application_adapter_execution_gate(
        adapter_execution_gate_id="adapter-execution-gate-1",
        approval_id="approval-1",
        adapter_execution_dry_run_id="adapter-execution-dry-run-1",
        adapter_execution_dry_run_audit_id="audit-adapter-execution-dry-run-1",
        adapter_runtime_preflight_id="adapter-runtime-preflight-1",
        adapter_wiring_id="adapter-wiring-1",
        adapter_code_change_id="adapter-code-change-1",
        adapter_implementation_preflight_id="adapter-implementation-preflight-1",
        ...
        rollback_plan_ref="docs/runbooks/sdk-write-runner-rollback.md",
        smoke_runbook_ref="docs/runbooks/sdk-write-runner-smoke.md",
        idempotency_key="adapter-execution-gate-1",
        idempotency_hash="hash-idempotency-execution-gate-1",
        execution_gate_hash="hash-adapter-execution-gate-1",
    ).to_dict()

    assert payload["operation"] == "runtime_flag_application_adapter_execution_gate_record"
    assert payload["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/execution-gate/record"
    )
    assert payload["request"]["adapter_execution_gate_id"] == "adapter-execution-gate-1"
    assert payload["request"]["approval_id"] == "approval-1"
    assert payload["request"]["adapter_execution_dry_run_id"] == "adapter-execution-dry-run-1"
    assert payload["request"]["adapter_execution_dry_run_audit_id"] == "audit-adapter-execution-dry-run-1"
    assert payload["request"]["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["request"]["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["request"]["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["request"]["execution_gate_plan_ref"] == (
        "docs/runbooks/sdk-write-runner-runtime-flag-adapter-execution-gate.md"
    )
    assert payload["request"]["execution_gate_hash"] == "hash-adapter-execution-gate-1"
    assert payload["request"]["dry_run"] is True
    assert payload["owner_gate"]["requires_approved_sdk_approval"] is True
    assert payload["owner_gate"]["requires_accepted_adapter_execution_dry_run"] is True
    assert payload["owner_gate"]["requires_adapter_execution_dry_run_audit"] is True
    assert payload["owner_gate"]["requires_runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["owner_gate"]["requires_adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["owner_gate"]["requires_adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["owner_gate"]["requires_execution_gate_plan_ref"] is True
    assert payload["owner_gate"]["requires_signature_or_hash"] is True
    assert payload["owner_gate"]["runtime_flag_enabled"] is False
    assert payload["owner_gate"]["flag_application_performed"] is False
    assert payload["owner_gate"]["implementation_enabled"] is False
    ...
    assert payload["owner_gate"]["instantiates_adapter_in_sdk_invoke"] is False
    assert payload["owner_gate"]["execution_dry_run_invoked"] is False
    assert payload["owner_gate"]["execution_gate_opened"] is False
    assert payload["mutation_performed"] is False
    assert payload["network_mutation_performed"] is False
    assert payload["file_mutation_performed"] is False
    assert payload["wired_into_sdk_invoke"] is False
...
def test_sdk_live_write_runner_execution_acceptance_is_safe() -> None:
    payload = ControlPlaneSDK().record_live_write_runner_execution_acceptance(
    ...
    assert payload["operation"] == "live_write_runner_execution_acceptance_record"
    assert payload["endpoint"] == "/api/v1/control-plane/sdk/live-write-runner/execution-acceptance/record"
    assert payload["request"]["execution_acceptance_id"] == "execution-acceptance-1"
    assert payload["request"]["adapter_execution_gate_id"] == "adapter-execution-gate-1"
    assert payload["request"]["adapter_execution_gate_audit_id"] == "audit-adapter-execution-gate-1"
    assert payload["request"]["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["request"]["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["request"]["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["request"]["runbook_acknowledged"] is True
    assert payload["request"]["rollback_plan_acknowledged"] is True
    assert payload["request"]["smoke_runbook_acknowledged"] is True
    assert payload["request"]["acceptance_hash"] == "hash-execution-acceptance-1"
    assert payload["request"]["dry_run"] is True
    assert payload["owner_gate"]["requires_approved_sdk_approval"] is True
    assert payload["owner_gate"]["requires_accepted_adapter_execution_gate"] is True
    assert payload["owner_gate"]["requires_adapter_execution_gate_audit"] is True
    assert payload["owner_gate"]["requires_execution_acceptance_plan_ref"] is True
    assert payload["owner_gate"]["requires_runbook_acknowledged"] is True
    assert payload["owner_gate"]["requires_rollback_plan_acknowledged"] is True
    assert payload["owner_gate"]["requires_smoke_runbook_acknowledged"] is True
    assert payload["owner_gate"]["requires_signature_or_hash"] is True
    assert payload["owner_gate"]["runtime_flag_enabled"] is False
    assert payload["owner_gate"]["flag_application_performed"] is False
    assert payload["owner_gate"]["implementation_enabled"] is False
    ...
    assert payload["owner_gate"]["agent_execution_enabled"] is False
    assert payload["owner_gate"]["runner_invoked"] is False
    assert payload["owner_gate"]["write_runner_invoked"] is False
    assert payload["owner_gate"]["live_execution_started"] is False
    assert payload["owner_gate"]["mark_executed"] is False
    assert payload["owner_gate"]["mutation_performed"] is False
    assert payload["owner_gate"]["network_mutation_performed"] is False
    assert payload["owner_gate"]["file_mutation_performed"] is False
    ...
    assert payload["network_mutation_performed"] is False
    assert payload["file_mutation_performed"] is False
    assert payload["channel_mutation_performed"] is False
    assert payload["write_runner_invoked"] is False
    assert payload["live_execution_started"] is False
    assert payload["adapter_execution_allowed"] is False


def test_sdk_live_write_runner_invocation_review_is_safe() -> None:
    payload = ControlPlaneSDK().record_live_write_runner_invocation_review(
        ...
        rollback_plan_acknowledged=True,
        smoke_runbook_acknowledged=True,
        idempotency_key="invocation-review-1",
        idempotency_hash="hash-idempotency-invocation-review-1",
        review_hash="hash-invocation-review-1",
    ).to_dict()

    assert payload["operation"] == "live_write_runner_invocation_review_record"
    assert payload["endpoint"] == "/api/v1/control-plane/sdk/live-write-runner/invocation-review/record"
    assert payload["request"]["invocation_review_id"] == "invocation-review-1"
    assert payload["request"]["execution_acceptance_id"] == "execution-acceptance-1"
    assert payload["request"]["execution_acceptance_audit_id"] == "audit-execution-acceptance-1"
    assert payload["request"]["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert payload["request"]["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert payload["request"]["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert payload["request"]["invocation_review_plan_ref"] == (
        "docs/runbooks/sdk-write-runner-live-invocation-review.md"
    )
    assert payload["request"]["runbook_acknowledged"] is True
    assert payload["request"]["rollback_plan_acknowledged"] is True
    assert payload["request"]["smoke_runbook_acknowledged"] is True
    assert payload["request"]["review_hash"] == "hash-invocation-review-1"
    assert payload["request"]["dry_run"] is True
    assert payload["owner_gate"]["requires_approved_sdk_approval"] is True
    assert payload["owner_gate"]["requires_accepted_execution_acceptance"] is True
    assert payload["owner_gate"]["requires_execution_acceptance_audit"] is True
    assert payload["owner_gate"]["requires_invocation_review_plan_ref"] is True
    assert payload["owner_gate"]["requires_signature_or_hash"] is True
    assert payload["owner_gate"]["runtime_flag_enabled"] is False
    assert payload["owner_gate"]["flag_application_performed"] is False
    assert payload["owner_gate"]["implementation_enabled"] is False
...
def test_cli_sdk_runtime_flag_application_adapter_code_change_execute_flag_records_code_change_only() -> None:
    set_current_config(CLIConfig(api_base_url="http://localhost:8000", output_format="json"))
    mock_client = AsyncMock()
    mock_client.record_sdk_runtime_flag_application_adapter_code_change.return_value = {
        "ok": True,
    ...
    with patch("cli.commands.sdk_cmd.create_client", return_value=mock_client):
        result = CliRunner().invoke(
            ...
            [
                "sdk",
                "runtime-flag-application-adapter-code-change-record",
                "--adapter-code-change-id",
                "adapter-code-change-1",
                "--approval-id",
                "approval-1",
                "--adapter-implementation-preflight-id",
                "adapter-implementation-preflight-1",
                "--adapter-implementation-preflight-audit-id",
                "audit-adapter-implementation-preflight-1",
                "--adapter-design-review-id",
                "adapter-design-review-1",
                "--adapter-implementation-request-id",
                ...
                "2026-06-08T00:00:00Z",
                "--implementation-branch-ref",
                "codex/runtime-flag-adapter-code-change",
                "--implementation-plan-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-implementation.md",
                "--adapter-design-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-design.md",
                ...
                "adapter-code-change-1",
                "--idempotency-hash",
                "hash-idempotency-code-change-1",
                "--code-change-hash",
                "hash-adapter-code-change-1",
                "--execute",
            ],
        )
    ...
    assert payload["adapter_code_change"]["record_status"] == "recorded"
    assert payload["adapter_code_change"]["runtime_flag_enabled"] is False
    assert payload["adapter_code_change"]["flag_application_performed"] is False
    assert payload["adapter_code_change"]["implementation_enabled"] is False
    assert payload["adapter_code_change"]["write_runner_enabled"] is False
    assert payload["adapter_code_change"]["adapter_execution_enabled"] is False
    assert payload["adapter_code_change"]["runner_invoked"] is False
    assert payload["adapter_code_change"]["mutation_performed"] is False
    assert payload["adapter_code_change"]["adapter_import_allowed"] is False
    assert payload["adapter_code_change"]["adapter_execution_allowed"] is False

    mock_client.record_sdk_runtime_flag_application_adapter_code_change.assert_awaited_once()
    request = mock_client.record_sdk_runtime_flag_application_adapter_code_change.await_args.args[0]
    assert request["adapter_code_change_id"] == "adapter-code-change-1"
    assert request["approval_id"] == "approval-1"
    assert request["adapter_implementation_preflight_id"] == "adapter-implementation-preflight-1"
    assert request["adapter_implementation_preflight_audit_id"] == "audit-adapter-implementation-preflight-1"
    assert request["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert request["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert request["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert request["idempotency_key"] == "adapter-code-change-1"
    assert request["idempotency_hash"] == "hash-idempotency-code-change-1"
    assert request["code_change_hash"] == "hash-adapter-code-change-1"
    assert request["dry_run"] is True
    mock_client.invoke_sdk_contract.assert_not_called()


def test_cli_sdk_runtime_flag_application_adapter_wiring_execute_flag_records_wiring_only() -> None:
    set_current_config(CLIConfig(api_base_url="http://localhost:8000", output_format="json"))
    mock_client = AsyncMock()
    mock_client.record_sdk_runtime_flag_application_adapter_wiring.return_value = {
        "ok": True,
    ...
    with patch("cli.commands.sdk_cmd.create_client", return_value=mock_client):
        result = CliRunner().invoke(
            ...
            [
                "sdk",
                "runtime-flag-application-adapter-wiring-record",
                "--adapter-wiring-id",
                "adapter-wiring-1",
                "--approval-id",
                "approval-1",
                "--adapter-code-change-id",
                "adapter-code-change-1",
                "--adapter-code-change-audit-id",
                "audit-adapter-code-change-1",
                "--adapter-implementation-preflight-id",
                "adapter-implementation-preflight-1",
                "--adapter-design-review-id",
                "adapter-design-review-1",
                ...
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-wiring.md",
                "--implementation-branch-ref",
                "codex/runtime-flag-adapter-wiring",
                "--implementation-plan-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-implementation.md",
                "--adapter-design-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-design.md",
                ...
                "adapter-wiring-1",
                "--idempotency-hash",
                "hash-idempotency-wiring-1",
                "--wiring-hash",
                "hash-adapter-wiring-1",
                "--execute",
            ],
        )
    ...
    assert payload["adapter_wiring"]["record_status"] == "recorded"
    assert payload["adapter_wiring"]["runtime_flag_enabled"] is False
    assert payload["adapter_wiring"]["flag_application_performed"] is False
    assert payload["adapter_wiring"]["implementation_enabled"] is False
    assert payload["adapter_wiring"]["write_runner_enabled"] is False
    assert payload["adapter_wiring"]["adapter_execution_enabled"] is False
    assert payload["adapter_wiring"]["runner_invoked"] is False
    assert payload["adapter_wiring"]["mutation_performed"] is False
    assert payload["adapter_wiring"]["adapter_import_allowed"] is False
    assert payload["adapter_wiring"]["adapter_execution_allowed"] is False
    assert payload["adapter_wiring"]["wired_into_sdk_invoke"] is False
    assert payload["adapter_wiring"]["adapter_runtime_wired"] is False

    mock_client.record_sdk_runtime_flag_application_adapter_wiring.assert_awaited_once()
    request = mock_client.record_sdk_runtime_flag_application_adapter_wiring.await_args.args[0]
    assert request["adapter_wiring_id"] == "adapter-wiring-1"
    assert request["approval_id"] == "approval-1"
    assert request["adapter_code_change_id"] == "adapter-code-change-1"
    assert request["adapter_code_change_audit_id"] == "audit-adapter-code-change-1"
    assert request["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert request["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert request["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert request["wiring_plan_ref"] == "docs/runbooks/sdk-write-runner-runtime-flag-adapter-wiring.md"
    assert request["idempotency_key"] == "adapter-wiring-1"
    assert request["idempotency_hash"] == "hash-idempotency-wiring-1"
    assert request["wiring_hash"] == "hash-adapter-wiring-1"
    assert request["dry_run"] is True
    mock_client.invoke_sdk_contract.assert_not_called()


def test_cli_sdk_runtime_flag_application_adapter_runtime_preflight_execute_flag_records_preflight_only() -> None:
    set_current_config(CLIConfig(api_base_url="http://localhost:8000", output_format="json"))
    mock_client = AsyncMock()
    mock_client.record_sdk_runtime_flag_application_adapter_runtime_preflight.return_value = {
        "ok": True,
    ...
    with patch("cli.commands.sdk_cmd.create_client", return_value=mock_client):
        result = CliRunner().invoke(
            ...
            [
                "sdk",
                "runtime-flag-application-adapter-runtime-preflight-record",
                "--adapter-runtime-preflight-id",
                "adapter-runtime-preflight-1",
                "--approval-id",
                "approval-1",
                "--adapter-wiring-id",
                "adapter-wiring-1",
                "--adapter-wiring-audit-id",
                "audit-adapter-wiring-1",
                "--adapter-code-change-id",
                "adapter-code-change-1",
                "--adapter-implementation-preflight-id",
                "adapter-implementation-preflight-1",
                ...
                "--preflighted-at",
                "2026-06-08T00:00:00Z",
                "--runtime-preflight-plan-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-runtime-preflight.md",
                "--wiring-plan-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-wiring.md",
                "--implementation-branch-ref",
                "codex/runtime-flag-adapter-runtime-preflight",
                "--implementation-plan-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-implementation.md",
                "--adapter-design-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-design.md",
                ...
                "adapter-runtime-preflight-1",
                "--idempotency-hash",
                "hash-idempotency-runtime-preflight-1",
                "--runtime-preflight-hash",
                "hash-adapter-runtime-preflight-1",
                "--execute",
            ],
        )
    ...
    assert payload["adapter_runtime_preflight"]["record_status"] == "recorded"
    assert payload["adapter_runtime_preflight"]["runtime_flag_enabled"] is False
    assert payload["adapter_runtime_preflight"]["flag_application_performed"] is False
    assert payload["adapter_runtime_preflight"]["implementation_enabled"] is False
    assert payload["adapter_runtime_preflight"]["write_runner_enabled"] is False
    assert payload["adapter_runtime_preflight"]["adapter_execution_enabled"] is False
    assert payload["adapter_runtime_preflight"]["runner_invoked"] is False
    assert payload["adapter_runtime_preflight"]["mutation_performed"] is False
    assert payload["adapter_runtime_preflight"]["adapter_import_allowed"] is False
    assert payload["adapter_runtime_preflight"]["adapter_execution_allowed"] is False
    assert payload["adapter_runtime_preflight"]["wired_into_sdk_invoke"] is False
    assert payload["adapter_runtime_preflight"]["adapter_runtime_wired"] is False

    mock_client.record_sdk_runtime_flag_application_adapter_runtime_preflight.assert_awaited_once()
    request = mock_client.record_sdk_runtime_flag_application_adapter_runtime_preflight.await_args.args[0]
    assert request["adapter_runtime_preflight_id"] == "adapter-runtime-preflight-1"
    assert request["approval_id"] == "approval-1"
    assert request["adapter_wiring_id"] == "adapter-wiring-1"
    assert request["adapter_wiring_audit_id"] == "audit-adapter-wiring-1"
    assert request["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert request["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert request["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert request["runtime_preflight_plan_ref"] == (
        "docs/runbooks/sdk-write-runner-runtime-flag-adapter-runtime-preflight.md"
    )
    assert request["idempotency_key"] == "adapter-runtime-preflight-1"
    assert request["idempotency_hash"] == "hash-idempotency-runtime-preflight-1"
    assert request["runtime_preflight_hash"] == "hash-adapter-runtime-preflight-1"
    assert request["dry_run"] is True
    mock_client.invoke_sdk_contract.assert_not_called()


def test_cli_sdk_runtime_flag_application_adapter_execution_dry_run_execute_flag_records_dry_run_only() -> None:
    set_current_config(CLIConfig(api_base_url="http://localhost:8000", output_format="json"))
    mock_client = AsyncMock()
    mock_client.record_sdk_runtime_flag_application_adapter_execution_dry_run.return_value = {
        "ok": True,
    ...
    with patch("cli.commands.sdk_cmd.create_client", return_value=mock_client):
        result = CliRunner().invoke(
            ...
            [
                "sdk",
                "runtime-flag-application-adapter-execution-dry-run-record",
                "--adapter-execution-dry-run-id",
                "adapter-execution-dry-run-1",
                "--approval-id",
                "approval-1",
                "--adapter-runtime-preflight-id",
                "adapter-runtime-preflight-1",
                "--adapter-runtime-preflight-audit-id",
                "audit-adapter-runtime-preflight-1",
                "--adapter-wiring-id",
                "adapter-wiring-1",
                "--adapter-code-change-id",
                "adapter-code-change-1",
                ...
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-wiring.md",
                "--implementation-branch-ref",
                "codex/runtime-flag-adapter-execution-dry-run",
                "--implementation-plan-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-implementation.md",
                "--adapter-design-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-design.md",
                ...
                "adapter-execution-dry-run-1",
                "--idempotency-hash",
                "hash-idempotency-execution-dry-run-1",
                "--execution-dry-run-hash",
                "hash-adapter-execution-dry-run-1",
                "--execute",
            ],
        )
    ...
    assert payload["adapter_execution_dry_run"]["record_status"] == "recorded"
    assert payload["adapter_execution_dry_run"]["runtime_flag_enabled"] is False
    assert payload["adapter_execution_dry_run"]["flag_application_performed"] is False
    assert payload["adapter_execution_dry_run"]["implementation_enabled"] is False
    assert payload["adapter_execution_dry_run"]["write_runner_enabled"] is False
    assert payload["adapter_execution_dry_run"]["adapter_execution_enabled"] is False
    assert payload["adapter_execution_dry_run"]["runner_invoked"] is False
    assert payload["adapter_execution_dry_run"]["mutation_performed"] is False
    assert payload["adapter_execution_dry_run"]["adapter_import_allowed"] is False
    assert payload["adapter_execution_dry_run"]["adapter_execution_allowed"] is False
    assert payload["adapter_execution_dry_run"]["wired_into_sdk_invoke"] is False
    assert payload["adapter_execution_dry_run"]["adapter_runtime_wired"] is False
    assert payload["adapter_execution_dry_run"]["execution_dry_run_invoked"] is False

    mock_client.record_sdk_runtime_flag_application_adapter_execution_dry_run.assert_awaited_once()
    request = mock_client.record_sdk_runtime_flag_application_adapter_execution_dry_run.await_args.args[0]
    assert request["adapter_execution_dry_run_id"] == "adapter-execution-dry-run-1"
    assert request["approval_id"] == "approval-1"
    assert request["adapter_runtime_preflight_id"] == "adapter-runtime-preflight-1"
    assert request["adapter_runtime_preflight_audit_id"] == "audit-adapter-runtime-preflight-1"
    assert request["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert request["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert request["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert request["execution_dry_run_plan_ref"] == (
        "docs/runbooks/sdk-write-runner-runtime-flag-adapter-execution-dry-run.md"
    )
    assert request["idempotency_key"] == "adapter-execution-dry-run-1"
    assert request["idempotency_hash"] == "hash-idempotency-execution-dry-run-1"
    assert request["execution_dry_run_hash"] == "hash-adapter-execution-dry-run-1"
    assert request["dry_run"] is True
    mock_client.invoke_sdk_contract.assert_not_called()


def test_cli_sdk_runtime_flag_application_adapter_execution_gate_execute_flag_records_gate_only() -> None:
    set_current_config(CLIConfig(api_base_url="http://localhost:8000", output_format="json"))
    mock_client = AsyncMock()
    mock_client.record_sdk_runtime_flag_application_adapter_execution_gate.return_value = {
        "ok": True,
    ...
    with patch("cli.commands.sdk_cmd.create_client", return_value=mock_client):
        result = CliRunner().invoke(
            ...
            [
                "sdk",
                "runtime-flag-application-adapter-execution-gate-record",
                "--adapter-execution-gate-id",
                "adapter-execution-gate-1",
                "--approval-id",
                "approval-1",
                "--adapter-execution-dry-run-id",
                "adapter-execution-dry-run-1",
                "--adapter-execution-dry-run-audit-id",
                "audit-adapter-execution-dry-run-1",
                "--adapter-runtime-preflight-id",
                "adapter-runtime-preflight-1",
                "--adapter-wiring-id",
                "adapter-wiring-1",
                ...
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-wiring.md",
                "--implementation-branch-ref",
                "codex/runtime-flag-adapter-execution-gate",
                "--implementation-plan-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-implementation.md",
                "--adapter-design-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-design.md",
                ...
                "adapter-execution-gate-1",
                "--idempotency-hash",
                "hash-idempotency-execution-gate-1",
                "--execution-gate-hash",
                "hash-adapter-execution-gate-1",
                "--execute",
            ],
        )
    ...
    assert payload["adapter_execution_gate"]["record_status"] == "recorded"
    assert payload["adapter_execution_gate"]["runtime_flag_enabled"] is False
    assert payload["adapter_execution_gate"]["flag_application_performed"] is False
    assert payload["adapter_execution_gate"]["implementation_enabled"] is False
    assert payload["adapter_execution_gate"]["write_runner_enabled"] is False
    assert payload["adapter_execution_gate"]["adapter_execution_enabled"] is False
    assert payload["adapter_execution_gate"]["runner_invoked"] is False
    assert payload["adapter_execution_gate"]["mutation_performed"] is False
    assert payload["adapter_execution_gate"]["adapter_import_allowed"] is False
    assert payload["adapter_execution_gate"]["adapter_execution_allowed"] is False
    assert payload["adapter_execution_gate"]["wired_into_sdk_invoke"] is False
    assert payload["adapter_execution_gate"]["adapter_runtime_wired"] is False
    assert payload["adapter_execution_gate"]["execution_dry_run_invoked"] is False
    assert payload["adapter_execution_gate"]["execution_gate_opened"] is False

    mock_client.record_sdk_runtime_flag_application_adapter_execution_gate.assert_awaited_once()
    request = mock_client.record_sdk_runtime_flag_application_adapter_execution_gate.await_args.args[0]
    assert request["adapter_execution_gate_id"] == "adapter-execution-gate-1"
    assert request["approval_id"] == "approval-1"
    assert request["adapter_execution_dry_run_id"] == "adapter-execution-dry-run-1"
    assert request["adapter_execution_dry_run_audit_id"] == "audit-adapter-execution-dry-run-1"
    assert request["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert request["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert request["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert request["execution_gate_plan_ref"] == (
        "docs/runbooks/sdk-write-runner-runtime-flag-adapter-execution-gate.md"
    )
    assert request["idempotency_key"] == "adapter-execution-gate-1"
    assert request["idempotency_hash"] == "hash-idempotency-execution-gate-1"
    assert request["execution_gate_hash"] == "hash-adapter-execution-gate-1"
    assert request["dry_run"] is True
    mock_client.invoke_sdk_contract.assert_not_called()


def test_cli_sdk_live_write_runner_execution_acceptance_execute_flag_records_acceptance_only() -> None:
    set_current_config(CLIConfig(api_base_url="http://localhost:8000", output_format="json"))
    mock_client = AsyncMock()
    mock_client.record_sdk_live_write_runner_execution_acceptance.return_value = {
        "ok": True,
    ...
    with patch("cli.commands.sdk_cmd.create_client", return_value=mock_client):
        result = CliRunner().invoke(
            ...
            [
                "sdk",
                "live-write-runner-execution-acceptance-record",
                "--execution-acceptance-id",
                "execution-acceptance-1",
                "--approval-id",
                "approval-1",
                "--adapter-execution-gate-id",
                "adapter-execution-gate-1",
                "--adapter-execution-gate-audit-id",
                "audit-adapter-execution-gate-1",
                "--adapter-execution-dry-run-id",
                "adapter-execution-dry-run-1",
                "--adapter-runtime-preflight-id",
                "adapter-runtime-preflight-1",
                ...
                "--accepted-at",
                "2026-06-08T00:00:00Z",
                "--execution-acceptance-plan-ref",
                "docs/runbooks/sdk-write-runner-live-execution-acceptance.md",
                "--execution-gate-plan-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-execution-gate.md",
                "--execution-dry-run-plan-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-execution-dry-run.md",
                ...
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-wiring.md",
                "--implementation-branch-ref",
                "codex/runtime-flag-adapter-execution-acceptance",
                "--implementation-plan-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-implementation.md",
                "--adapter-design-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-design.md",
                ...
                "execution-acceptance-1",
                "--idempotency-hash",
                "hash-idempotency-execution-acceptance-1",
                "--acceptance-hash",
                "hash-execution-acceptance-1",
                "--execute",
            ],
        )
    ...
    assert payload["execution_acceptance"]["record_status"] == "recorded"
    assert payload["execution_acceptance"]["runtime_flag_enabled"] is False
    assert payload["execution_acceptance"]["flag_application_performed"] is False
    assert payload["execution_acceptance"]["implementation_enabled"] is False
    assert payload["execution_acceptance"]["write_runner_enabled"] is False
    assert payload["execution_acceptance"]["adapter_execution_enabled"] is False
    assert payload["execution_acceptance"]["runner_invoked"] is False
    assert payload["execution_acceptance"]["write_runner_invoked"] is False
    assert payload["execution_acceptance"]["live_execution_started"] is False
    assert payload["execution_acceptance"]["mutation_performed"] is False
    assert payload["execution_acceptance"]["adapter_import_allowed"] is False
    assert payload["execution_acceptance"]["adapter_execution_allowed"] is False
    assert payload["execution_acceptance"]["wired_into_sdk_invoke"] is False
    assert payload["execution_acceptance"]["adapter_runtime_wired"] is False
    assert payload["execution_acceptance"]["execution_dry_run_invoked"] is False
    assert payload["execution_acceptance"]["execution_gate_opened"] is False

    mock_client.record_sdk_live_write_runner_execution_acceptance.assert_awaited_once()
    request = mock_client.record_sdk_live_write_runner_execution_acceptance.await_args.args[0]
    assert request["execution_acceptance_id"] == "execution-acceptance-1"
    assert request["approval_id"] == "approval-1"
    assert request["adapter_execution_gate_id"] == "adapter-execution-gate-1"
    assert request["adapter_execution_gate_audit_id"] == "audit-adapter-execution-gate-1"
    assert request["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert request["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert request["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert request["execution_acceptance_plan_ref"] == (
        "docs/runbooks/sdk-write-runner-live-execution-acceptance.md"
    )
    assert request["runbook_acknowledged"] is True
    assert request["rollback_plan_acknowledged"] is True
    assert request["smoke_runbook_acknowledged"] is True
    assert request["idempotency_key"] == "execution-acceptance-1"
    assert request["idempotency_hash"] == "hash-idempotency-execution-acceptance-1"
    assert request["acceptance_hash"] == "hash-execution-acceptance-1"
    assert request["dry_run"] is True
    mock_client.invoke_sdk_contract.assert_not_called()


def test_cli_sdk_live_write_runner_invocation_review_execute_flag_records_review_only() -> None:
    set_current_config(CLIConfig(api_base_url="http://localhost:8000", output_format="json"))
    mock_client = AsyncMock()
    mock_client.record_sdk_live_write_runner_invocation_review.return_value = {
        "ok": True,
    ...
    with patch("cli.commands.sdk_cmd.create_client", return_value=mock_client):
        result = CliRunner().invoke(
            ...
            [
                "sdk",
                "live-write-runner-invocation-review-record",
                "--invocation-review-id",
                "invocation-review-1",
                "--approval-id",
                "approval-1",
                "--execution-acceptance-id",
                "execution-acceptance-1",
                "--execution-acceptance-audit-id",
                "audit-execution-acceptance-1",
                "--adapter-execution-gate-id",
                "adapter-execution-gate-1",
                "--adapter-execution-gate-audit-id",
                "audit-adapter-execution-gate-1",
                ...
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-wiring.md",
                "--implementation-branch-ref",
                "codex/runtime-flag-adapter-invocation-review",
                "--implementation-plan-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-implementation.md",
                "--adapter-design-ref",
                "docs/runbooks/sdk-write-runner-runtime-flag-adapter-design.md",
                ...
                "invocation-review-1",
                "--idempotency-hash",
                "hash-idempotency-invocation-review-1",
                "--review-hash",
                "hash-invocation-review-1",
                "--execute",
            ],
        )
    ...
    assert payload["invocation_review"]["record_status"] == "recorded"
    assert payload["invocation_review"]["runtime_flag_enabled"] is False
    assert payload["invocation_review"]["flag_application_performed"] is False
    assert payload["invocation_review"]["implementation_enabled"] is False
    assert payload["invocation_review"]["write_runner_enabled"] is False
    assert payload["invocation_review"]["adapter_execution_enabled"] is False
    assert payload["invocation_review"]["runner_invoked"] is False
    assert payload["invocation_review"]["write_runner_invoked"] is False
    assert payload["invocation_review"]["live_execution_started"] is False
    assert payload["invocation_review"]["mutation_performed"] is False
    assert payload["invocation_review"]["adapter_import_allowed"] is False
    assert payload["invocation_review"]["adapter_execution_allowed"] is False
    assert payload["invocation_review"]["wired_into_sdk_invoke"] is False
    assert payload["invocation_review"]["adapter_runtime_wired"] is False
    assert payload["invocation_review"]["execution_dry_run_invoked"] is False
    assert payload["invocation_review"]["execution_gate_opened"] is False

    mock_client.record_sdk_live_write_runner_invocation_review.assert_awaited_once()
    request = mock_client.record_sdk_live_write_runner_invocation_review.await_args.args[0]
    assert request["invocation_review_id"] == "invocation-review-1"
    assert request["approval_id"] == "approval-1"
    assert request["execution_acceptance_id"] == "execution-acceptance-1"
    assert request["execution_acceptance_audit_id"] == "audit-execution-acceptance-1"
    assert request["adapter_execution_gate_id"] == "adapter-execution-gate-1"
    assert request["adapter_execution_gate_audit_id"] == "audit-adapter-execution-gate-1"
    assert request["runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert request["adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert request["adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert request["invocation_review_plan_ref"] == (
        "docs/runbooks/sdk-write-runner-live-invocation-review.md"
    )
    assert request["runbook_acknowledged"] is True
    assert request["rollback_plan_acknowledged"] is True
    assert request["smoke_runbook_acknowledged"] is True
    assert request["idempotency_key"] == "invocation-review-1"
    assert request["idempotency_hash"] == "hash-idempotency-invocation-review-1"
    assert request["review_hash"] == "hash-invocation-review-1"
    assert request["dry_run"] is True
    mock_client.invoke_sdk_contract.assert_not_called()
