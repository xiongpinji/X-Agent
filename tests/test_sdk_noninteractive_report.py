...
def test_sdk_noninteractive_report_default_is_read_only() -> None:
    ...
    assert report.status == "sdk_live_write_runner_invocation_review_workflow_ready"
...
def test_sdk_noninteractive_report_covers_sdk_and_cli_methods() -> None:
    ...
    assert any(
        item["operation"] == "runtime_flag_application_readiness_plan_decision_record"
        for item in report.sdk_contracts
    )
    assert any(
        item["operation"] == "runtime_flag_application_adapter_implementation_request_record"
        for item in report.sdk_contracts
    )
    assert any(
        item["operation"] == "runtime_flag_application_adapter_design_review_record"
        for item in report.sdk_contracts
    )
    assert any(
    ...
    assert any(
        item["operation"] == "runtime_flag_application_adapter_wiring_record"
        for item in report.sdk_contracts
    )
    assert any(
        item["operation"] == "runtime_flag_application_adapter_runtime_preflight_record"
        for item in report.sdk_contracts
    )
    assert any(
        item["operation"] == "runtime_flag_application_adapter_execution_dry_run_record"
        for item in report.sdk_contracts
    )
    assert all(
...
def test_sdk_noninteractive_report_covers_runtime_flag_application_owner_approval_workflow() -> None:
    workflow = build_sdk_noninteractive_report().runtime_flag_application_owner_approval_workflow

    assert workflow["stage"] == "runtime_flag_application_owner_approval_record_workflow"
    assert workflow["workflow_status"] == "ready_but_disabled"
    assert workflow["endpoint"] == "/api/v1/control-plane/sdk/runtime-flag/application-approval/record"
    assert workflow["sdk_operation"] == "runtime_flag_application_owner_approval_record"
    assert workflow["audit_action"] == "sdk.write_runner.runtime_flag_application_owner_approval_recorded"
    assert workflow["requires_approved_sdk_approval"] is True
    assert workflow["requires_runtime_flag_application_preflight"] is True
    assert workflow["requires_decision_accept_or_reject"] is True
    assert workflow["requires_runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert workflow["requires_signature_or_hash"] is True
    assert workflow["audit_event_recorded_by_sdk_invoke"] is False
...
def test_sdk_noninteractive_report_covers_runtime_flag_application_execute_contract_owner_review() -> None:
    review = build_sdk_noninteractive_report().runtime_flag_application_execute_contract_owner_review

    assert review["stage"] == "runtime_flag_application_execute_contract_owner_review"
    assert review["review_status"] == "ready_but_disabled"
    assert review["review_pack_type"] == "sdk_write_runner_runtime_flag_application_execute_contract_owner_review"
    assert review["source_workflow"] == "runtime_flag_application_execute_contract_record_workflow"
    assert "sdk.write_runner.runtime_flag_application_execute_contract_recorded" in review["required_audit_actions"]
    assert review["owner_review_policy"]["manual_review_required"] is True
    assert review["owner_review_policy"]["independent_review_required"] is True
    assert review["owner_review_policy"]["requires_live_application_request"] is True
    assert review["owner_review_policy"]["can_apply_runtime_flag_after_review"] is False
    assert review["owner_review_policy"]["can_invoke_write_runner_after_review"] is False
    assert review["review_readback"]["method"] == "runtime/evidence/read"
    assert review["review_readback"]["evidence_type"] == (
        "sdk_write_runner_runtime_flag_application_execute_contract"
    )
    assert review["implementation_enabled"] is False
    assert review["runtime_flag_enabled"] is False
...
def test_sdk_noninteractive_report_covers_live_runtime_flag_application_readiness_plan_decision_workflow() -> None:
    workflow = build_sdk_noninteractive_report().live_runtime_flag_application_readiness_plan_decision_workflow

    assert workflow["stage"] == "runtime_flag_application_readiness_plan_decision_record_workflow"
    assert workflow["workflow_status"] == "ready_but_disabled"
    assert workflow["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-readiness-plan/decision/record"
    )
    assert workflow["sdk_operation"] == "runtime_flag_application_readiness_plan_decision_record"
    assert workflow["audit_action"] == (
        "sdk.write_runner.runtime_flag_application_readiness_plan_decision_recorded"
    )
    assert workflow["requires_approved_sdk_approval"] is True
    assert workflow["requires_runtime_flag_application_execute_contract"] is True
    assert workflow["requires_readiness_plan_review"] is True
    assert workflow["requires_decision_accept_or_reject"] is True
    assert workflow["requires_runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert workflow["requires_signature_or_hash"] is True
    assert workflow["decision_effect"]["accepted_applies_runtime_flag"] is False
    assert workflow["decision_effect"]["accepted_enables_adapter"] is False
    assert workflow["decision_effect"]["accepted_invokes_write_runner"] is False
    assert workflow["implementation_enabled"] is False
    assert workflow["runtime_flag_enabled"] is False
    assert workflow["flag_application_performed"] is False
...
def test_sdk_noninteractive_report_covers_live_runtime_flag_application_adapter_implementation_request_workflow() -> None:
    workflow = build_sdk_noninteractive_report().live_runtime_flag_application_adapter_implementation_request_workflow

    assert workflow["stage"] == "runtime_flag_application_adapter_implementation_request_workflow"
    assert workflow["workflow_status"] == "ready_but_disabled"
    assert workflow["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/implementation-request/record"
    )
    assert workflow["sdk_operation"] == "runtime_flag_application_adapter_implementation_request_record"
    assert workflow["audit_action"] == (
        "sdk.write_runner.runtime_flag_application_adapter_implementation_request_recorded"
    )
    assert workflow["requires_approved_sdk_approval"] is True
    assert workflow["requires_accepted_readiness_plan_decision"] is True
    assert workflow["requires_readiness_plan_decision_audit"] is True
    assert workflow["requires_runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert workflow["requires_adapter_design_ref"] is True
    assert workflow["requires_rollback_plan_ref"] is True
    assert workflow["requires_smoke_runbook_ref"] is True
    assert workflow["requires_signature_or_hash"] is True
    assert workflow["request_effect"]["implements_adapter"] is False
    assert workflow["request_effect"]["imports_adapter"] is False
    assert workflow["request_effect"]["instantiates_adapter"] is False
    assert workflow["request_effect"]["applies_runtime_flag"] is False
    assert workflow["request_effect"]["invokes_write_runner"] is False
    assert workflow["implementation_enabled"] is False
    assert workflow["runtime_flag_enabled"] is False
    assert workflow["flag_application_performed"] is False
    assert workflow["execute_enabled"] is False
...
def test_sdk_noninteractive_report_covers_live_runtime_flag_application_adapter_design_review_workflow() -> None:
    workflow = build_sdk_noninteractive_report().live_runtime_flag_application_adapter_design_review_workflow

    assert workflow["stage"] == "runtime_flag_application_adapter_design_review_workflow"
    assert workflow["workflow_status"] == "ready_but_disabled"
    assert workflow["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/design-review/record"
    )
    assert workflow["sdk_operation"] == "runtime_flag_application_adapter_design_review_record"
    assert workflow["audit_action"] == (
        "sdk.write_runner.runtime_flag_application_adapter_design_review_recorded"
    )
    assert workflow["requires_approved_sdk_approval"] is True
    assert workflow["requires_accepted_adapter_implementation_request"] is True
    assert workflow["requires_adapter_implementation_request_audit"] is True
    assert workflow["requires_runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert workflow["requires_review_decision_accept_or_reject"] is True
    assert workflow["requires_adapter_design_ref"] is True
    assert workflow["requires_security_review_ref"] is True
    assert workflow["requires_test_plan_ref"] is True
    assert workflow["requires_rollback_plan_ref"] is True
    assert workflow["requires_smoke_runbook_ref"] is True
    assert workflow["requires_signature_or_hash"] is True
    assert workflow["review_effect"]["accepted_implements_adapter"] is False
    assert workflow["review_effect"]["accepted_imports_adapter"] is False
    assert workflow["review_effect"]["accepted_instantiates_adapter"] is False
    assert workflow["review_effect"]["accepted_applies_runtime_flag"] is False
    assert workflow["review_effect"]["accepted_invokes_write_runner"] is False
    assert workflow["implementation_enabled"] is False
    assert workflow["runtime_flag_enabled"] is False
    assert workflow["flag_application_performed"] is False
    assert workflow["execute_enabled"] is False
...
def test_sdk_noninteractive_report_covers_live_runtime_flag_application_adapter_implementation_preflight_workflow() -> None:
    workflow = (
        build_sdk_noninteractive_report()
        .live_runtime_flag_application_adapter_implementation_preflight_workflow
    )

    assert workflow["stage"] == "runtime_flag_application_adapter_implementation_preflight_workflow"
    assert workflow["workflow_status"] == "ready_but_disabled"
    assert workflow["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/implementation-preflight/record"
    )
    assert workflow["sdk_operation"] == "runtime_flag_application_adapter_implementation_preflight_record"
    assert workflow["audit_action"] == (
        "sdk.write_runner.runtime_flag_application_adapter_implementation_preflight_recorded"
    )
    assert workflow["requires_approved_sdk_approval"] is True
    assert workflow["requires_accepted_adapter_design_review"] is True
    assert workflow["requires_adapter_design_review_audit"] is True
    assert workflow["requires_runtime_flag_name"] == "XAGENT_SDK_WRITE_RUNNER_ENABLED"
    assert workflow["requires_implementation_branch_ref"] is True
    assert workflow["requires_implementation_plan_ref"] is True
    assert workflow["requires_adapter_design_ref"] is True
    assert workflow["requires_security_review_ref"] is True
    assert workflow["requires_test_plan_ref"] is True
    assert workflow["requires_rollback_plan_ref"] is True
    ...
    assert workflow["requires_idempotency_hash"] is True
    assert workflow["requires_signature_or_hash"] is True
    assert workflow["preflight_effect"]["implements_adapter"] is False
    assert workflow["preflight_effect"]["imports_adapter"] is False
    assert workflow["preflight_effect"]["writes_adapter_files"] is False
    assert workflow["preflight_effect"]["invokes_write_runner"] is False
    assert workflow["implementation_enabled"] is False
    assert workflow["runtime_flag_enabled"] is False
    assert workflow["flag_application_performed"] is False
    assert workflow["execute_enabled"] is False
...
def test_sdk_noninteractive_report_covers_live_runtime_flag_application_adapter_wiring_workflow() -> None:
    workflow = build_sdk_noninteractive_report().live_runtime_flag_application_adapter_wiring_workflow

    assert workflow["stage"] == "runtime_flag_application_adapter_wiring_workflow"
    assert workflow["workflow_status"] == "ready_but_disabled"
    assert workflow["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/wiring/record"
    )
    assert workflow["sdk_operation"] == "runtime_flag_application_adapter_wiring_record"
    assert workflow["audit_action"] == "sdk.write_runner.runtime_flag_application_adapter_wiring_recorded"
    assert workflow["requires_approved_sdk_approval"] is True
    assert workflow["requires_accepted_adapter_code_change"] is True
    assert workflow["requires_adapter_code_change_audit"] is True
    assert workflow["requires_adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert workflow["requires_adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert workflow["requires_wiring_plan_ref"] is True
    assert workflow["wiring_effect"]["wiring_recorded"] is True
    assert workflow["wiring_effect"]["wired_into_sdk_invoke"] is False
    assert workflow["wiring_effect"]["adapter_runtime_wired"] is False
    assert workflow["wiring_effect"]["imports_adapter_in_sdk_invoke"] is False
    assert workflow["wiring_effect"]["instantiates_adapter_in_sdk_invoke"] is False
    assert workflow["wiring_effect"]["invokes_write_runner"] is False
    assert workflow["implementation_enabled"] is False
    assert workflow["runtime_flag_enabled"] is False
    assert workflow["flag_application_performed"] is False
    assert workflow["execute_enabled"] is False
...
def test_sdk_noninteractive_report_covers_live_runtime_flag_application_adapter_runtime_preflight_workflow() -> None:
    workflow = build_sdk_noninteractive_report().live_runtime_flag_application_adapter_runtime_preflight_workflow

    assert workflow["stage"] == "runtime_flag_application_adapter_runtime_preflight_workflow"
    assert workflow["workflow_status"] == "ready_but_disabled"
    assert workflow["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/runtime-preflight/record"
    )
    assert workflow["sdk_operation"] == "runtime_flag_application_adapter_runtime_preflight_record"
    assert workflow["audit_action"] == "sdk.write_runner.runtime_flag_application_adapter_runtime_preflight_recorded"
    assert workflow["requires_approved_sdk_approval"] is True
    assert workflow["requires_accepted_adapter_wiring"] is True
    assert workflow["requires_adapter_wiring_audit"] is True
    assert workflow["requires_adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert workflow["requires_adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert workflow["requires_runtime_preflight_plan_ref"] is True
    assert workflow["runtime_preflight_effect"]["runtime_preflight_recorded"] is True
    assert workflow["runtime_preflight_effect"]["wired_into_sdk_invoke"] is False
    assert workflow["runtime_preflight_effect"]["adapter_runtime_wired"] is False
    assert workflow["runtime_preflight_effect"]["imports_adapter_in_sdk_invoke"] is False
    assert workflow["runtime_preflight_effect"]["instantiates_adapter_in_sdk_invoke"] is False
    assert workflow["runtime_preflight_effect"]["invokes_write_runner"] is False
    assert workflow["implementation_enabled"] is False
    assert workflow["runtime_flag_enabled"] is False
    assert workflow["flag_application_performed"] is False
    assert workflow["execute_enabled"] is False
...
def test_sdk_noninteractive_report_covers_live_runtime_flag_application_adapter_execution_dry_run_workflow() -> None:
    workflow = build_sdk_noninteractive_report().live_runtime_flag_application_adapter_execution_dry_run_workflow

    assert workflow["stage"] == "runtime_flag_application_adapter_execution_dry_run_workflow"
    assert workflow["workflow_status"] == "ready_but_disabled"
    assert workflow["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/execution-dry-run/record"
    )
    assert workflow["sdk_operation"] == "runtime_flag_application_adapter_execution_dry_run_record"
    assert workflow["audit_action"] == "sdk.write_runner.runtime_flag_application_adapter_execution_dry_run_recorded"
    assert workflow["requires_approved_sdk_approval"] is True
    assert workflow["requires_accepted_adapter_runtime_preflight"] is True
    assert workflow["requires_adapter_runtime_preflight_audit"] is True
    assert workflow["requires_adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert workflow["requires_adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert workflow["requires_execution_dry_run_plan_ref"] is True
    assert workflow["execution_dry_run_effect"]["execution_dry_run_recorded"] is True
    assert workflow["execution_dry_run_effect"]["wired_into_sdk_invoke"] is False
    assert workflow["execution_dry_run_effect"]["adapter_runtime_wired"] is False
    assert workflow["execution_dry_run_effect"]["imports_adapter_in_sdk_invoke"] is False
    assert workflow["execution_dry_run_effect"]["instantiates_adapter_in_sdk_invoke"] is False
    assert workflow["execution_dry_run_effect"]["invokes_write_runner"] is False
    assert workflow["execution_dry_run_effect"]["execution_dry_run_invoked"] is False
    assert workflow["implementation_enabled"] is False
    assert workflow["runtime_flag_enabled"] is False
    assert workflow["flag_application_performed"] is False
    assert workflow["execute_enabled"] is False
...
def test_sdk_noninteractive_report_covers_live_runtime_flag_application_adapter_execution_gate_workflow() -> None:
    workflow = build_sdk_noninteractive_report().live_runtime_flag_application_adapter_execution_gate_workflow

    assert workflow["stage"] == "runtime_flag_application_adapter_execution_gate_workflow"
    assert workflow["workflow_status"] == "ready_but_disabled"
    assert workflow["endpoint"] == (
        "/api/v1/control-plane/sdk/runtime-flag/application-adapter/execution-gate/record"
    )
    assert workflow["sdk_operation"] == "runtime_flag_application_adapter_execution_gate_record"
    assert workflow["audit_action"] == "sdk.write_runner.runtime_flag_application_adapter_execution_gate_recorded"
    assert workflow["requires_approved_sdk_approval"] is True
    assert workflow["requires_accepted_adapter_execution_dry_run"] is True
    assert workflow["requires_adapter_execution_dry_run_audit"] is True
    assert workflow["requires_adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert workflow["requires_adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert workflow["requires_execution_gate_plan_ref"] is True
    assert workflow["execution_gate_effect"]["execution_gate_recorded"] is True
    assert workflow["execution_gate_effect"]["execution_gate_opened"] is False
    assert workflow["execution_gate_effect"]["wired_into_sdk_invoke"] is False
    assert workflow["execution_gate_effect"]["adapter_runtime_wired"] is False
    assert workflow["execution_gate_effect"]["imports_adapter_in_sdk_invoke"] is False
    assert workflow["execution_gate_effect"]["instantiates_adapter_in_sdk_invoke"] is False
    assert workflow["execution_gate_effect"]["invokes_write_runner"] is False
    assert workflow["execution_gate_effect"]["execution_dry_run_invoked"] is False
    assert workflow["implementation_enabled"] is False
    assert workflow["runtime_flag_enabled"] is False
    assert workflow["flag_application_performed"] is False
    assert workflow["execute_enabled"] is False
...
def test_sdk_noninteractive_report_covers_live_write_runner_invocation_review_workflow() -> None:
    workflow = build_sdk_noninteractive_report().live_write_runner_invocation_review_workflow

    assert workflow["stage"] == "live_write_runner_invocation_review_workflow"
    assert workflow["workflow_status"] == "ready_but_disabled"
    assert workflow["endpoint"] == "/api/v1/control-plane/sdk/live-write-runner/invocation-review/record"
    assert workflow["sdk_operation"] == "live_write_runner_invocation_review_record"
    assert workflow["audit_action"] == "sdk.write_runner.live_write_runner_invocation_review_recorded"
    assert workflow["requires_approved_sdk_approval"] is True
    assert workflow["requires_accepted_execution_acceptance"] is True
    assert workflow["requires_execution_acceptance_audit"] is True
    assert workflow["requires_adapter_module"] == "backend.app.sdk.runtime_flag_application_adapter"
    assert workflow["requires_adapter_class"] == "SDKRuntimeFlagApplicationAdapter"
    assert workflow["requires_invocation_review_plan_ref"] is True
    assert workflow["requires_execution_acceptance_plan_ref"] is True
    assert workflow["requires_runbook_acknowledged"] is True
    assert workflow["requires_rollback_plan_acknowledged"] is True
    assert workflow["requires_smoke_runbook_acknowledged"] is True
    assert workflow["invocation_review_effect"]["invocation_review_recorded"] is True
    assert workflow["invocation_review_effect"]["execution_gate_opened"] is False
    assert workflow["invocation_review_effect"]["wired_into_sdk_invoke"] is False
    assert workflow["invocation_review_effect"]["adapter_runtime_wired"] is False
    assert workflow["invocation_review_effect"]["imports_adapter_in_sdk_invoke"] is False
    assert workflow["invocation_review_effect"]["instantiates_adapter_in_sdk_invoke"] is False
    assert workflow["invocation_review_effect"]["invokes_write_runner"] is False
    assert workflow["invocation_review_effect"]["live_execution_started"] is False
    assert workflow["invocation_review_effect"]["write_runner_invoked"] is False
    assert workflow["implementation_enabled"] is False
    assert workflow["runtime_flag_enabled"] is False
    assert workflow["flag_application_performed"] is False
    assert workflow["execute_enabled"] is False
...
def test_write_sdk_noninteractive_report_json_and_markdown(tmp_path: Path) -> None:
    ...
    assert "## Live Runtime Flag Application Readiness Plan Decision Workflow" in markdown
    ...
    assert "## Live Runtime Flag Application Adapter Wiring Workflow" in markdown
    ...
    assert "## Live Write Runner Invocation Review Workflow" in markdown
