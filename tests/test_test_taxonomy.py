...
def test_test_file_naming_matches_taxonomy() -> None:
    ...
    for path in files:
        ...
        if parts[0] == "e2e":
            ...
            # test_smoke_api.py 为冒烟 API 用例，test_user_journey.py 为用户旅程用例，
            # 同属命名债白名单。
            assert name.endswith("_e2e.py") or name in {'test_agent_fix_real_llm.py', 'test_desktop_e2e.py', 'test_desktop_macro_e2e.py', 'test_execution_reporting.py', 'test_functional_e2e.py', 'test_offline_e2e.py', 'test_open_source_catalog_e2e.py', 'test_open_source_e2e.py', 'test_performance_security_e2e.py', 'test_smoke_api.py', 'test_sync_e2e.py', 'test_user_journey.py', 'test_user_journey_api.py', 'test_workflow_e2e.py'}
