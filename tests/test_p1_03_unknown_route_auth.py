...
def _app_with_router(module_name: str, *, role: str = "viewer") -> FastAPI:
    ...
    async def _principal_middleware(request, call_next):  # type: ignore[no-untyped-def]
        request.scope["principal"] = _Principal(role=role)
...
def test_p1_03_admin_can_reach_enterprise_partner_plugin_unknown_routes(
    module_name: str,
    method: str,
    path: str,
    payload: dict[str, Any],
) -> None:
    client = TestClient(_app_with_router(module_name, role="admin"), raise_server_exceptions=False)
...
def test_p1_03_developer_can_reach_i18n_unknown_write_routes(
    method: str,
    path: str,
    payload: dict[str, Any] | None,
) -> None:
    client = TestClient(_app_with_router("backend.app.api.i18n", role="developer"), raise_server_exceptions=False)
...
def test_p1_03_new_public_route_set_is_limited_to_auth_entrypoints_and_i18n_reads() -> None:
    from scripts.route_auth_audit import PUBLIC_ROUTES

    p1_public_routes = {
        route
        for route in PUBLIC_ROUTES
        if "/enterprise/sso" in route[1] or route[1].startswith("/api/i18n/")
    }

    assert p1_public_routes == {
