...
class DocsOpenSourceProvider:
    ...
    def search(self, query: str, limit: int = 10) -> list[OpenSourceCandidateRecord]:
        try:
            ...
            return []
        sites = ["https://playwright.dev", "https://qdrant.tech", "https://docs.python.org/3/"]
        ...
        for site in sites:
            ...
            try:
                ...
                title = (soup.title.text.strip() if soup.title and soup.title.text else site).strip()
                text = " ".join(soup.get_text(" ", strip=True).split())
                snippet = text[:500]
                if q and q not in (title + " " + snippet).lower():
                    continue
                results.append(OpenSourceCandidateRecord(name=title, source=self.name, url=site, summary=snippet, score=0.72, reasons=["documentation source"], tags=["docs", "reference"], metadata={"provider": self.name, "provider_kind": "docs", "title": title, "content_length": len(text)}))
            except Exception:
                continue
        return results
