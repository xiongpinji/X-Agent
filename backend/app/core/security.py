...
ROLE_SCOPES: dict[str, list[str]] = {
    "admin": [
        ...
        "scheduler:manage",
        "scheduler:read",
        # 2026-09-16 组织域读写端点配套（POST /api/v1/organization/agents、
        # /organizations、/departments）。拍板 Q8：org:write 给 admin + developer
        # —— 在控制台建岗位智能体属于开发者日常操作，只给 admin 会让页面又一次
        # 「看得见、点不动」。
        # org:read 此前**没有任何角色持有**（organization_control 的 4 个 GET 依赖
        # 它，一旦挂载必然 403）；随本轮组织/部门查询端点一并登记。
        "org:read",
        "org:write",
    ],
    "developer": [
        ...
        "scheduler:manage",
        "scheduler:read",
        # 2026-09-16 组织域读写端点配套：与 admin 同口径（见上方注释）。
        "org:read",
        "org:write",
    ],
    "user": [
